...
export interface PriceContext {
  ...
  spark1m: number[]      // 최근 1개월(≈5주) 주봉 종가 — 초단기 추세 미니차트용
}
    ...
    for (const sym of tries) {
      try {
        ...
        if (closes.length >= 10) {
          ...
          const spark1m = closes.slice(-5).map(v => Math.round(v * 100) / 100)     // 최근 ≈1개월 주봉
          return { price: Math.round(price * 100) / 100, low52: Math.round(low52 * 100) / 100, high52: Math.round(high52 * 100) / 100, posPct, spark, spark3m, spark1m }
        }
      } catch { /* 다음 심볼 */ }
    }
    return null
