...
export async function generateBriefing(decision: SignalDecision, m: SignalMetrics, recs: Recommendation[]): Promise<BriefingText> {
  ...
  const fallback = (): BriefingText => {
    ...
    // ⚖️ 아첨 금지 — 신호 방향만 응원하지 않도록 반대편도 한 줄 병기(균형)
    const balance = isSell
      ? ' 다만 장기 추세·실적 반등 여지가 남았다면 무조건 팔 자리는 아니니, 보유 이유가 아직 유효한지 함께 점검하세요.'
      : ' 다만 고평가 전환·추세 이탈·실적 훼손 가능성 같은 반대 리스크가 없는지 함께 확인하세요.'
    return {
      title: `${m.name} ${isSell ? '리스크 점검 필요' : '저평가 기회 포착'}`,
      content: `${isSell ? '매도를 검토할 신호입니다' : '매수 기회 신호입니다'}: ${decision.reasons.join(' · ')}.${recTxt}${balance} 단정하지 말고, 회사의 실제 실적이 이 신호를 뒷받침하는지 직접 확인하세요.`,
    }
  }
...
- ⚖️ 아첨 금지·양면 균형: 신호 방향만 응원하는 치어리더가 되지 말 것. 3문장 중 최소 1문장은 반대편을 반드시 짚어라 — ${isSell ? "매도 신호라도 '그래도 계속 보유할 이유가 있다면 무엇인지'(예: 장기 추세·실적 반등 여지)를 1개 병기" : "매수 신호라도 '이 매수를 무효화할 반대 리스크가 무엇인지'(예: 고평가 전환·추세 이탈·실적 훼손 가능성)를 1개 병기"}. 학생이 듣고 싶은 말이 아니라 스스로 판단하도록 균형된 재료를 준다.
