...
export const SCREEN_SETUPS: SetupMeta[] = [
  ...
  { key: 'pullback', icon: '🎼', label: '첫 눌림목', edge20: 1.41, winRate: 51.9, sample: 567,
    ...
    // ⚠️ 재측정(2026-07-29·54종목·1,053건): 평균 edge +1.39%p 로 위 수치와 일관하나,
    //    **상하위 10% 제거 시 −0.45%p** 로 우위가 사라진다 = 소수 대박에 의존한다.
    //    교과서가 말하는 '조정 시 거래량 마름' 조건을 붙여도 개선 없음(−0.46%p·표본만 58% 감소).
    //    오히려 거래량이 터진 눌림목이 승률 61.7%·중위 2.20% 로 더 좋았다(교과서와 반대).
    //    ⭐ 단, **정예 타점(눌림목 × 정배열+구름)은 이상치 제거 후에도 +1.8%p 로 살아남는다**
    //       — 눌림목 단독이 아니라 '구조 게이트와의 합류'가 우위의 실체다.
    note: '⚠️ 단독으로는 우위가 얕습니다 — 상하위 10% 이상치를 걷어내면 초과수익이 사라집니다(소수 대박 의존). 🏅 정예 타점(정배열+구름 위와 겹칠 때)만 이상치 제거 후에도 우위가 남았습니다.' },
