...
export interface SeasonMeta {
  ...
  /** 현금 운용 방향(조언 텍스트 — 점수 항 아님).
   *  ⚠️ 구체 % 를 쓰지 마라(2026-08-30 감사) — 권장 현금 밴드의 SSOT 는 cashPosition.cashBandOf 이고,
   *  여기서 따로 % 를 단정해 두 화면이 서로 다른 '권장 현금'을 말하고 있었다(0~10% vs 30~40%). */
  cashHint: string
...
export const SEASON_META: Record<Quadrant, SeasonMeta> = {
  goldilocks: {
    ...
    cashHint: '유동성이 도는 국면이라 현금을 줄이고 주식을 가동할 시기입니다(구체 비중은 자산관리의 현금 포지션 카드 참고).',
  ...
  inflation: {
    ...
    cashHint: '이익이 정점을 향하는 공격적 구간입니다 — 현금 비중을 낮게 가져갈 만한 국면입니다.',
  ...
  stagflation: {
    ...
    cashHint: '위험 자산을 줄이고 현금을 확보할 국면입니다(구체 비중은 자산관리의 현금 포지션 카드 참고).',
  ...
  recession: {
    ...
    cashHint: '방어적 포지션을 유지할 국면입니다 — 현금 비중을 높여둘 만합니다.',
  ...
  shoulder: {
    ...
    cashHint: '방향이 불명확하니 보수적으로 운용하세요 — 현금을 여유 있게 두는 편이 낫습니다.',
