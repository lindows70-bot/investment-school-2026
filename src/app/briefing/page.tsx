...
export default function BriefingPage() {
  ...
  // 🪙 비트코인 한 줄 — 급등락 배너는 당일 ±5% 만 봐서 3개월 +37% 같은 꾸준한 흐름은 브리핑에 안 떴다(2026-09-24). 캐시만 읽는 요약(외부 호출 0)
  const coin = useFetch<{ usd: number | null; krw: number | null; change: { w1: number | null; m1: number | null; m3: number | null }; fng: number | null; answer: { line: string; action: string; verdict: string }; tone: 'accumulate' | 'caution' | 'neutral' } | null>('/api/coin-brief')
  ...
  return (
    <div style={{ padding: '20px 22px', display: 'flex', flexDirection: 'column', gap: 12, maxWidth: 1100, margin: '0 auto' }}>
      ...
      <Section no="④" title="판 읽기" sub="섹터 자금 순환 — 돈이 어디서 나와 어디로 가나" link="/dashboard?tab=rotation" linkLabel="로테이션 시계 상세">
        ...
        {/* 🪙 비트코인 한 줄 — 코인 랩의 '답 한 줄'(측정값만 · 처방과 같은 임계)을 그대로. 등락은 한국식(빨강=상승) */}
        {coin.d && coin.d.answer && (
          <div style={{ marginTop: 9, background: TK.bg3, border: `1px solid ${BORDER}`, borderRadius: 9, padding: '8px 11px', fontSize: FS.tiny, color: TK.slate300, lineHeight: 1.7 }}>
            🪙 <b style={{ color: TK.slate200 }}>비트코인</b>
            {coin.d.krw != null && <> ₩{Math.round(coin.d.krw).toLocaleString()}</>}
            {coin.d.change.m1 != null && <> · 1개월 <b style={{ color: coin.d.change.m1 >= 0 ? TK.red400 : TK.blue400 }}>{coin.d.change.m1 >= 0 ? '+' : ''}{coin.d.change.m1}%</b></>}
            {coin.d.change.m3 != null && <> · 3개월 <b style={{ color: coin.d.change.m3 >= 0 ? TK.red400 : TK.blue400 }}>{coin.d.change.m3 >= 0 ? '+' : ''}{coin.d.change.m3}%</b></>}
            {' '}— <b style={{ color: coin.d.tone === 'accumulate' ? TK.green400 : coin.d.tone === 'caution' ? TK.orange400 : TK.amber400 }}>{coin.d.answer.verdict}</b>
            {' '}<a href="/dashboard?tab=coinlab&cv=btc" style={{ color: TK.indigo400, textDecoration: 'none', fontWeight: 700 }}>코인 랩 →</a>
            <div style={{ color: TK.sub3 }}>{coin.d.answer.action}</div>
          </div>
        )}
      </Section>
