...
function lynchMsg(sig: CrossSignal, name: string, gap: number, con: number): string {
  ...
  if (sig === 'DANGER')
    return con >= 2
      // ⚠️ '린치는 2분기 연속 시 즉시 매도 권고'는 원전에 없는 창작 귀속이었다(2026-08-30 감사).
      //    린치가 재고 증가를 경고 신호로 든 것은 사실이나, '2분기=즉시 매도'는 이 앱이 정한 기준이다.
      ? `"${name}의 재고가 매출보다 ${a}%p 빠르게 쌓이는 상황이 ${con}분기 연속입니다. 린치는 재고가 매출보다 빨리 늘어나는 것을 경고 신호로 봤고, ${con}분기 연속을 위험으로 보는 기준은 이 앱이 정한 것입니다."`
