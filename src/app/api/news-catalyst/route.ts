...
function buildPrompt(ticker: string, name: string, market: string, headlines: string[], peg: number | null, tier: ValuationTier, opMargin: number | null, psr: number | null): string {
  ...
  // PSR: 적자기업은 PER/PEG가 무의미 → 매출 대비 밸류(P/S)가 유일한 가격 잣대. 단 절대 임계는 산업마다 달라 금지 — AI가 매출성장률과 함께 판단.
  const psrStr = psr != null ? ` · PSR ${psr.toFixed(1)}배(매출 대비 — 절대 고저 단정 말고 매출 성장률과 함께 해석)` : ''
  const valuationLine = tier === 'LOSS'
    ? `영업적자 기업${omStr}${psrStr} — 순이익 기반 PEG는 무의미하다. 매출은 거짓말 안 하니 PSR로 '매출 대비 얼마나 비싼가'를 보되, 현금 소진·자본 잠식 리스크가 본질. (Jarvis 처방전상 "매도 검토" 가능 구간)`
    : peg != null
...
다음 매트릭스를 따라 학생에게 균형 잡힌 행동을 1~3문장으로 제시하라:
   - [영업적자(LOSS) + 뉴스 호재]: ⚠️ 가장 조심할 경우. "기술·매출 성장은 고무적이나 아직 영업적자(현금 소진) 기업이다. 좋은 뉴스에 취해 추격매수하지 말고, 흑자 전환·현금 런웨이를 먼저 확인하라. 린치는 '아직 증명되지 않은 회생주'엔 신중했다" (Tempus TEM이 이 경우). ※ PSR이 주어졌다면: 매출이 폭발하는데 PSR이 낮으면 '과매도된 성장주'일 수 있고, 매출 성장은 둔한데 PSR만 높으면 '스토리 거품'이다 — 둘을 함께 언급하라.
...
async function analyzeTicker(
  ...
  const prompt = buildPrompt(ticker, name, market, headlines, peg, valuationTier, fund?.opMargin ?? null, fund?.psr ?? null)
