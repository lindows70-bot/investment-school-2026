...
export async function GET(req: Request) {
  ...
  const key = `market-flow-kr-v1:${new Date(Date.now() + 9 * 3600_000).toISOString().slice(0, 10)}`
  if (result.poolSize > 0) await setCache(key, result)
  return NextResponse.json(
    { ok: true, poolSize: result.poolSize, foreignTop: result.foreignTop.slice(0, 3).map(e => e.name), ms: Date.now() - t0 },
