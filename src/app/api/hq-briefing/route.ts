...
export async function GET(req: Request) {
  ...
  const cacheKey = `hq-briefing-v5:${user.id}:${kstDate()}:${fp}`   // v5: 연준 기조(고용 균열+워시 절사평균) 참고 맥락 주입
  ...
  const [season, unified, rebal, fed] = await Promise.all([
  ...
  if (fed) {
    ...
    const label = tilt === 'dovish' ? '비둘기 우위(완화 쪽)' : tilt === 'hawkish' ? '매파 우위(긴축 유지)' : '중립(데이터 의존)'
    const note = `고용 ${fed.laborStatus === 'hot' ? '과열' : fed.laborStatus === 'cooling' ? '둔화' : '균형(연착륙)'}, ` +
      `워시 기조물가(절사평균) ${fed.trimmedPce.latest}% vs 헤드라인 ${fed.headlinePce}%(노이즈 ${fed.noiseGap}%p)` +
      (tilt === 'dovish' ? ' → 헤드라인이 부풀어 연준이 시장 기대보다 인하에 열려 있을 수 있음(성장·기술주에 우호적 가능성).'
