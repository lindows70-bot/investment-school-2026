...
import MastersCommittee   from '@/app/components/MastersCommittee'
...
type ResearchTab = 'chart' | 'lynch' | 'buffett' | 'choi' | 'masters' | 'report'
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 14, fontFamily: '-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif' }}>
      ...
      <div style={{ display: 'flex', gap: 6, borderBottom: `1px solid ${TK.line4}`, paddingBottom: 0 }}>
        {([
          ...
          { key: 'masters', label: '🎩 거장 위원회',   desc: '버핏·멍거·단요핑·리루 4인 판정+토론' },
            ...
            <div style={{ background: N, boxShadow: SHO, borderRadius: 14, padding: '16px' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
                {stockInfo && [
                  ...
                  // 📈 라벨이 값을 따라간다 — Yahoo 폴백 경로(네이버 미커버 신규 상장주)에서 이익 자료가
                  //    없으면 **매출** 성장률로 대체된다(실측 2026-08-16: IONQ 286.8% 가 'EPS 성장률'로 표시).
                  //    폴백 자체는 합리적이지만 라벨을 안 바꾸면 학생이 이익과 매출을 같은 잣대로 읽는다.
      ...
      {/* ── 🎩 거장 위원회 탭 — 4인 독립 판정 → 교차 반박 → 강제 결론 ── */}
      {activeTab === 'masters' && (!stockInfo ? GuruEmptyNotice : !isStock ? NonStockNotice : (
        <MastersCommittee ticker={stockInfo.ticker} name={stockInfo.name} market={stockInfo.market} />
      ))}
