...
const TAG_COLOR: Record<string, string> = {
  '공지': TK.blue400, '필독': TK.red500, '일정': TK.amber400,
}
...
export default function SchoolLoungePage() {
  ...
  if (authLoading) return (
    <div style={{ padding: 60, textAlign: 'center', color: TK.sub4, fontFamily: 'sans-serif' }}>인증 확인 중…</div>
  )
  ...
  return (
    <div style={{ padding: '36px 28px 60px', fontFamily: '-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif', color: TK.sub12, maxWidth: 1100 }}>
      ...
      <div style={{ marginBottom: 24 }}>
        <div style={{ fontSize: 11, fontWeight: 700, color: TK.amber500, letterSpacing: '0.14em', textTransform: 'uppercase', marginBottom: 8 }}>💬 SCHOOL LOUNGE</div>
        <h1 style={{ fontSize: 26, fontWeight: 900, color: TK.slate100, margin: 0, letterSpacing: '-0.4px' }}>투자학교 학생 커뮤니티</h1>
      </div>
      ...
      <div style={{ background: 'linear-gradient(135deg,#1e1b4b,#312e81)', border: '1px solid #4338ca', borderRadius: 14, padding: '16px 22px', marginBottom: 26, display: 'flex', alignItems: 'center', gap: 16, boxShadow: '0 0 20px rgba(99,102,241,.2)' }}>
        ...
        <div style={{ flex: 1 }}>
          <div style={{ fontSize: 10, fontWeight: 700, color: TK.indigo400, letterSpacing: '0.1em', textTransform: 'uppercase', marginBottom: 4 }}>교장의 한마디</div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '290px 1fr', gap: 20, alignItems: 'start' }}>
        ...
        <div>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
            <span style={{ fontSize: 13, fontWeight: 800, color: TK.sub12 }}>📌 공지사항</span>
            {me?.isAdmin && (
              <button onClick={() => setShowAddNotice(!showAddNotice)} style={{ display: 'flex', alignItems: 'center', gap: 4, padding: '5px 10px', borderRadius: 7, border: 'none', cursor: 'pointer', background: N, boxShadow: SHO, color: TK.amber400, fontSize: 12, fontWeight: 700 }}>
                <IcoPlus/> 추가
              </button>
            )}
          </div>
          ...
          {me?.isAdmin && showAddNotice && (
            <div style={{ background: N, boxShadow: SHO, borderRadius: 12, padding: 16, marginBottom: 12, animation: 'fadeIn .2s' }}>
              <div style={{ fontSize: 11, fontWeight: 700, color: TK.amber400, letterSpacing: '0.08em', marginBottom: 10 }}>📌 새 공지 작성</div>
              <select value={noticeTag} onChange={e => setNoticeTag(e.target.value)}
                style={{ width: '100%', background: TK.bg0, boxShadow: SHI, border: 'none', borderRadius: 7, color: TK.sub12, padding: '7px 10px', fontSize: 12, marginBottom: 8, cursor: 'pointer' }}>
              ...
              <input placeholder="공지 제목 *" value={noticeTitle} onChange={e => setNoticeTitle(e.target.value)}
                style={{ width: '100%', background: TK.bg0, boxShadow: SHI, border: 'none', borderRadius: 7, color: TK.sub12, padding: '8px 10px', fontSize: 12, marginBottom: 8, boxSizing: 'border-box' as const }}/>
              <textarea placeholder="본문 내용 (선택 — 상세 내용을 입력하세요)" value={noticeContent}
                onChange={e => setNoticeContent(e.target.value)} rows={4}
                style={{ width: '100%', background: TK.bg0, boxShadow: SHI, border: 'none', borderRadius: 7, color: TK.sub12, padding: '8px 10px', fontSize: 12, marginBottom: 10, boxSizing: 'border-box' as const, resize: 'vertical', lineHeight: 1.6 }}/>
              <div style={{ display: 'flex', gap: 7 }}>
                <button onClick={addNotice} disabled={busy || !noticeTitle.trim()}
                  style={{ flex: 1, padding: '8px', borderRadius: 7, border: 'none', cursor: noticeTitle.trim() ? 'pointer' : 'not-allowed', background: `linear-gradient(135deg,#1e3a8a,${TK.blue500})`, color: '#fff', fontSize: 12, fontWeight: 700, opacity: noticeTitle.trim() ? 1 : 0.5 }}>등록</button>
                <button onClick={() => { setShowAddNotice(false); setNoticeTitle(''); setNoticeContent('') }}
                  style={{ padding: '8px 14px', borderRadius: 7, border: 'none', cursor: 'pointer', background: TK.bg0, boxShadow: SHI, color: TK.sub, fontSize: 12 }}>취소</button>
              </div>
            </div>
          )}
          ...
          {me?.isAdmin && editNotice && (
            <div style={{ background: N, boxShadow: SHO, borderRadius: 12, padding: 16, marginBottom: 12, border: '1px solid rgba(251,191,36,.3)', animation: 'fadeIn .2s' }}>
              <div style={{ fontSize: 11, fontWeight: 700, color: TK.amber400, letterSpacing: '0.08em', marginBottom: 10 }}>✏️ 공지 수정</div>
              ...
              <div style={{ display: 'flex', gap: 7 }}>
                <button onClick={saveEditNotice} disabled={busy || !editNotice.title.trim()}
                  style={{ flex: 1, padding: '8px', borderRadius: 7, border: 'none', cursor: 'pointer', background: `linear-gradient(135deg,#78350f,${TK.amber500})`, color: '#fff', fontSize: 12, fontWeight: 700 }}>저장</button>
          ...
          {dataLoading ? (
            ...
            <div style={{ textAlign: 'center', color: TK.sub10, fontSize: 12, padding: '24px 0' }}>공지사항이 없습니다</div>
          ) : notices.map(n => (
            <div key={n.id} style={{ background: N, boxShadow: SHO, borderRadius: 11, marginBottom: 9, animation: 'fadeIn .3s', overflow: 'hidden' }}>
              ...
              <div style={{ padding: '11px 13px', display: 'flex', alignItems: 'flex-start', gap: 9 }}>
                <span style={{ fontSize: 10, fontWeight: 700, color: TAG_COLOR[n.tag] ?? TK.blue400, background: `${TAG_COLOR[n.tag] ?? TK.blue400}18`, border: `1px solid ${TAG_COLOR[n.tag] ?? TK.blue400}44`, borderRadius: 5, padding: '2px 7px', flexShrink: 0, marginTop: 1 }}>{n.tag}</span>
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ fontSize: 12, fontWeight: 600, color: TK.sub12, lineHeight: 1.4, marginBottom: 3 }}>
                    {n.title}
                    {n.is_edited && <span style={{ fontSize: 9, color: TK.sub7, marginLeft: 5 }}>(수정됨)</span>}
                  </div>
                  <div style={{ fontSize: 10, color: TK.sub4 }}>{fmtDate(n.created_at)}</div>
                </div>
                {me?.isAdmin && (
                  <div style={{ display: 'flex', gap: 4, flexShrink: 0 }}>
                    <button
                      onClick={() => { setEditNotice({ id: n.id, title: n.title, content: n.content ?? '', tag: n.tag }); setShowAddNotice(false) }}
                      style={{ background: 'none', border: 'none', cursor: 'pointer', color: TK.sub7, padding: 2 }}
                      onMouseEnter={e => (e.currentTarget.style.color=TK.amber400)}
                      onMouseLeave={e => (e.currentTarget.style.color=TK.sub7)}>
                      <IcoPen/>
                    </button>
                    <button onClick={() => deleteNotice(n.id)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: TK.sub7, padding: 2 }} onMouseEnter={e => (e.currentTarget.style.color=TK.red400)} onMouseLeave={e => (e.currentTarget.style.color=TK.sub7)}>
                      <IcoTrash/>
                    </button>
                  </div>
                )}
              ...
              {n.content && (
                <div style={{ padding: '0 13px 13px 13px', borderTop: '1px solid rgba(255,255,255,0.05)', paddingTop: 10 }}>
                  <div style={{ fontSize: 12, color: TK.slate400, lineHeight: 1.7, whiteSpace: 'pre-wrap' as const }}>{n.content}</div>
                </div>
              )}
            </div>
          ))}
        ...
        <div>
          ...
          {me && (
            <div style={{ background: N, boxShadow: SHO, borderRadius: 14, padding: 16, marginBottom: 18 }}>
              <textarea placeholder="투자 인사이트를 공유해보세요... 종목 분석, 시장 의견, 질문 모두 환영합니다 💬"
                ...
                style={{ width: '100%', background: TK.bg0, boxShadow: SHI, border: 'none', borderRadius: 9, color: TK.sub12, padding: '11px 13px', fontSize: 13, resize: 'none', lineHeight: 1.6, boxSizing: 'border-box' as const }}/>
              <div style={{ display: 'flex', justifyContent: 'flex-end', marginTop: 10 }}>
                <button onClick={addPost} disabled={!newPost.trim() || busy}
                  style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '9px 18px', borderRadius: 9, border: 'none', cursor: newPost.trim() ? 'pointer' : 'not-allowed', background: newPost.trim() ? `linear-gradient(135deg,#78350f,${TK.amber500})` : TK.bg0, boxShadow: newPost.trim() ? '0 4px 14px rgba(245,158,11,.3)' : SHI, color: newPost.trim() ? '#fff' : TK.sub4, fontSize: 13, fontWeight: 700, transition: 'all .2s' }}>
                  <IcoSend/> 게시하기
                </button>
              </div>
            </div>
          ...
          {dataLoading ? (
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
              {posts.length === 0 && <div style={{ textAlign: 'center', color: TK.sub10, fontSize: 13, padding: '40px 0' }}><div style={{ fontSize: 32, marginBottom: 10 }}>💬</div>첫 번째 인사이트를 공유해보세요!</div>}
              ...
              {posts.map(post => (
                <div key={post.id} style={{ background: N, boxShadow: post.is_admin_post ? `${SHO},0 0 0 1px rgba(251,191,36,.35)` : SHO, borderRadius: 14, border: post.is_admin_post ? '1px solid rgba(251,191,36,.3)' : 'none', animation: post.is_admin_post ? 'glow 3s infinite' : 'fadeIn .3s', overflow: 'hidden' }}>
                  ...
                  <div style={{ padding: '16px 18px' }}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 10 }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 9 }}>
                        <div style={{ width: 32, height: 32, borderRadius: 9, background: post.is_admin_post ? `linear-gradient(135deg,${TK.amber800},${TK.amber700})` : `linear-gradient(135deg,#1e3a5f,${TK.blue600})`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 13, fontWeight: 800, color: '#fff', flexShrink: 0 }}>
                          {post.author_name[0]}
                        </div>
                        <div>
                          <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                            <span style={{ fontSize: 13, fontWeight: 700, color: TK.slate100 }}>{post.author_name}</span>
                            {post.is_admin_post && <span style={{ fontSize: 9, fontWeight: 800, color: TK.amber400, background: 'rgba(251,191,36,.12)', border: '1px solid rgba(251,191,36,.4)', borderRadius: 4, padding: '1px 6px' }}>교장</span>}
                            {post.is_edited && <span style={{ fontSize: 9, color: TK.sub4 }}>수정됨</span>}
                          </div>
                          <div style={{ fontSize: 10, color: TK.sub4 }}>{fmtDate(post.created_at)}</div>
                        </div>
                      </div>
                      {/* 수정/삭제 버튼 */}
                      <div style={{ display: 'flex', gap: 4 }}>
                        {canEdit(post.user_id) && !editPost && (
                          <button onClick={() => setEditPost({ id: post.id, val: post.content })} style={{ background: 'none', border: 'none', cursor: 'pointer', color: TK.sub4, padding: 4 }} onMouseEnter={e => (e.currentTarget.style.color=TK.blue400)} onMouseLeave={e => (e.currentTarget.style.color=TK.sub4)}><IcoPen/></button>
                        )}
                        {canDelete(post.user_id) && (
                          <button onClick={() => deletePost(post.id)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: TK.sub4, padding: 4 }} onMouseEnter={e => (e.currentTarget.style.color=TK.red400)} onMouseLeave={e => (e.currentTarget.style.color=TK.sub4)}><IcoTrash/></button>
                        )}
                      </div>
                    </div>
                    ...
                    {editPost?.id === post.id ? (
                      <div>
                        <textarea value={editPost.val} onChange={e => setEditPost({ ...editPost, val: e.target.value })} rows={3}
                          style={{ width: '100%', background: TK.bg0, boxShadow: SHI, border: 'none', borderRadius: 9, color: TK.sub12, padding: '10px 12px', fontSize: 13, resize: 'none', marginBottom: 8, boxSizing: 'border-box' as const }}/>
                        <div style={{ display: 'flex', gap: 8 }}>
                          <button onClick={saveEditPost} style={{ padding: '7px 16px', borderRadius: 8, border: 'none', cursor: 'pointer', background: `linear-gradient(135deg,#1e3a8a,${TK.blue500})`, color: '#fff', fontSize: 12, fontWeight: 700 }}>저장</button>
                          <button onClick={() => setEditPost(null)} style={{ padding: '7px 12px', borderRadius: 8, border: 'none', cursor: 'pointer', background: TK.bg0, boxShadow: SHI, color: TK.sub, fontSize: 12 }}>취소</button>
                        </div>
                      </div>
                    ) : (
                      <p style={{ fontSize: 13, color: TK.sub12, lineHeight: 1.7, margin: '0 0 12px', whiteSpace: 'pre-wrap' }}>{post.content}</p>
                    )}

                    {/* 답글 토글 버튼 */}
                    {!editPost && (
                      <button onClick={() => setOpenThread(openThread === post.id ? null : post.id)}
                        style={{ display: 'flex', alignItems: 'center', gap: 5, background: 'none', border: 'none', cursor: 'pointer', color: openThread === post.id ? TK.blue400 : TK.sub4, fontSize: 12, fontWeight: 600, padding: 0 }}>
                  ...
                  {/* 스레드 */}
                  {openThread === post.id && (
                    <div style={{ borderTop: `1px solid ${TK.bg9}`, background: TK.bg0, padding: '14px 18px' }}>
                      {post.comments.map(c => (
                        <div key={c.id} style={{ display: 'flex', gap: 9, marginBottom: 12, animation: 'fadeIn .2s' }}>
                          <div style={{ width: 26, height: 26, borderRadius: 7, background: `linear-gradient(135deg,#1e3a5f,${TK.blue600})`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 11, fontWeight: 800, color: '#fff', flexShrink: 0 }}>{c.author_name[0]}</div>
                          <div style={{ flex: 1, background: N, borderRadius: 9, padding: '9px 12px' }}>
                            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 5 }}>
                              <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                                <span style={{ fontSize: 12, fontWeight: 700, color: TK.sub12 }}>{c.author_name}</span>
                                {c.is_edited && <span style={{ fontSize: 9, color: TK.sub4 }}>수정됨</span>}
                                <span style={{ fontSize: 10, color: TK.sub10 }}>{fmtDate(c.created_at)}</span>
                              </div>
                              <div style={{ display: 'flex', gap: 4 }}>
                                {canEdit(c.user_id) && !editComment && (
                                  <button onClick={() => setEditComment({ id: c.id, postId: post.id, val: c.content })} style={{ background: 'none', border: 'none', cursor: 'pointer', color: TK.sub4, padding: 2 }} onMouseEnter={e => (e.currentTarget.style.color=TK.blue400)} onMouseLeave={e => (e.currentTarget.style.color=TK.sub4)}><IcoPen/></button>
                                )}
                                {canDelete(c.user_id) && (
                                  <button onClick={() => deleteComment(post.id, c.id)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: TK.sub4, padding: 2 }} onMouseEnter={e => (e.currentTarget.style.color=TK.red400)} onMouseLeave={e => (e.currentTarget.style.color=TK.sub4)}><IcoTrash/></button>
                                )}
                              </div>
                            </div>
                            {editComment?.id === c.id ? (
                              <div>
                                <input value={editComment.val} onChange={e => setEditComment({ ...editComment, val: e.target.value })}
                                  style={{ width: '100%', background: TK.bg0, boxShadow: SHI, border: 'none', borderRadius: 7, color: TK.sub12, padding: '7px 10px', fontSize: 12, marginBottom: 7, boxSizing: 'border-box' as const }}/>
                                <div style={{ display: 'flex', gap: 6 }}>
                                  <button onClick={saveEditComment} style={{ padding: '5px 12px', borderRadius: 7, border: 'none', cursor: 'pointer', background: `linear-gradient(135deg,#1e3a8a,${TK.blue500})`, color: '#fff', fontSize: 11, fontWeight: 700 }}>저장</button>
                                  <button onClick={() => setEditComment(null)} style={{ padding: '5px 10px', borderRadius: 7, border: 'none', cursor: 'pointer', background: TK.bg2, color: TK.sub, fontSize: 11 }}>취소</button>
                                </div>
                              </div>
                            ) : (
                              <p style={{ fontSize: 12, color: TK.slate400, margin: 0, lineHeight: 1.6 }}>{c.content}</p>
                            )}
                          </div>
                        </div>
                      ))}
                      ...
                      {me && (
                        <div style={{ display: 'flex', gap: 8, alignItems: 'flex-end' }}>
                          <textarea placeholder="답글을 입력하세요..." value={replyInputs[post.id] ?? ''} onChange={e => setReplyInputs({ ...replyInputs, [post.id]: e.target.value })} rows={2}
                            style={{ flex: 1, background: N, boxShadow: SHI, border: 'none', borderRadius: 8, color: TK.sub12, padding: '9px 12px', fontSize: 12, resize: 'none', lineHeight: 1.5 }}/>
                          <button onClick={() => addComment(post.id)} style={{ display: 'flex', alignItems: 'center', gap: 5, padding: '9px 14px', borderRadius: 8, border: 'none', cursor: 'pointer', background: `linear-gradient(135deg,#1e3a5f,${TK.blue500})`, color: '#fff', fontSize: 12, fontWeight: 700, flexShrink: 0 }}>
                            <IcoSend/> 등록
                          </button>
                        </div>
                      )}
