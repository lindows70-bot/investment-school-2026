...
// ═══════════════════════════════════════════════════════════════
//  TIME-SERIES MOCK DATA (2022-01 ~ 2024-12)
// ═══════════════════════════════════════════════════════════════
...
const MONTHS = genMonths('2022-01', 36)  // 36개월
...
const US_RATE = [
  ...
  5.33,5.33,5.33,5.33,5.33,5.08,4.83,4.58,4.58,4.58,4.33,4.33,
]
// NVDA 주가 (USD) — 근사치
const NVDA = [
  240,245,210,200,175,145,165,170,128,132,140,146,
  150,210,280,320,410,430,490,500,480,520,580,490,
  510,600,700,800,880,1080,1210,1100,950,880,800,870,
]
// PLTR 주가 (USD)
const PLTR = [
  13.5,12.8,11.2,10.4,9.1,8.2,9.4,9.8,8.1,7.2,7.8,8.0,
  9.0,11.2,13.0,14.5,15.2,14.8,14.2,15.0,16.5,17.2,18.0,17.0,
  18.5,19.8,21.0,22.5,24.0,26.5,28.0,25.0,32.0,38.0,42.0,50.0,
]
// 한국 CPI (YoY %)
const KR_CPI = [
  3.6,3.7,4.1,4.8,5.4,6.0,6.3,5.7,5.6,5.7,5.0,5.0,
  5.2,4.8,4.2,3.7,3.3,2.7,2.3,2.0,1.9,1.8,3.4,3.2,
  2.8,2.9,3.1,3.0,2.7,2.6,2.5,2.4,2.3,2.2,2.1,2.0,
]
// 포트폴리오 배당수익률 (%) — mock
const PORTFOLIO_DIV = [
...
const PRESETS = {
  rate_vs_nvda: {
    ...
    right: { key:'nvda',  label:'NVDA ($)',           color:D.neon,   data:NVDA,    unit:'$',  yDomain:[100,1300] },
  },
  cpi_vs_div: {
    label: '인플레 vs 배당',
    icon: '💰',
    desc: '한국 CPI vs 내 배당수익률',
    left:  { key:'cpi', label:'한국 CPI (%)',         color:D.orange, data:KR_CPI,       unit:'%', yDomain:[0,8]   },
    right: { key:'div', label:'포트폴리오 배당 (%)',  color:D.neon,   data:PORTFOLIO_DIV, unit:'%', yDomain:[0,4]  },
  },
  nvda_vs_pltr: {
    label: 'AI 주도주 대결',
    icon: '⚡',
    desc: 'NVDA vs PLTR 누적 수익률',
    left:  { key:'nvda_ret', label:'NVDA 누적수익률', color:D.neon,   data:NVDA.map(v=>parseFloat(((v/NVDA[0]-1)*100).toFixed(1))),  unit:'%', yDomain:[-50,400] },
    right: { key:'pltr_ret', label:'PLTR 누적수익률', color:D.blue,   data:PLTR.map(v=>parseFloat(((v/PLTR[0]-1)*100).toFixed(1))),  unit:'%', yDomain:[-50,400] },
  },
} as const
    ...
    <div style={{ background:D.surface, border:`1px solid ${D.border}`, borderRadius:18, overflow:'hidden' }}>
      ...
      <div style={{ padding:'16px 20px 10px', display:'flex', alignItems:'flex-start', justifyContent:'space-between', flexWrap:'wrap', gap:10 }}>
        <div>
          ...
          <div style={{ fontSize:12, color:D.sub, marginTop:3 }}>듀얼 Y축 비교 분석 · 2022–2024</div>
        </div>
        {/* 프리셋 버튼 */}
      ...
      {(() => {
        const insights: Record<PresetKey, { title:string; text:string }> = {
          rate_vs_nvda: {
            title: '📚 투자 인사이트',
            text: '2022년 금리 급등(0%→5.5%) 시 NVDA는 -50% 폭락했지만, AI 붐으로 2023년 역대급 반등. 금리와 기술주는 통상 역상관 관계이나 강력한 모멘텀이 이를 돌파하기도 합니다.',
          },
          cpi_vs_div: {
            title: '📚 투자 인사이트',
    ...
    <div style={{
      ...
      <div style={{ marginBottom:20, display:'flex', alignItems:'flex-end', justifyContent:'space-between', flexWrap:'wrap', gap:10 }}>
        ...
        <div style={{ fontSize:10, color:D.muted, textAlign:'right' as const }}>
          ...
          <div style={{ marginTop:2 }}>시계열: 2022.01 – 2024.12</div>
        </div>
      </div>
