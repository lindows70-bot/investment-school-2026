...
import { fetchUsdKrw, USD_KRW_FALLBACK } from '@/lib/fx'   // 💱 환율 SSOT(폴백 여부까지 — 폴백이면 캐시하지 않는다)
...
export interface PeerMetric {
  ...
  pe:        number | null    // 주가수익비율 PER — Yahoo trailingPE(최근 4분기 이익 기준). 양수만(적자·이상값은 null)
  isTarget:  boolean
}
...
// 💱 라이브 환율(fetchUsdKrw)을 getSectorPeers 가 한 번 받아 넘긴다 — 예전엔 상수(1,400)만 썼다(제1원칙)
...
async function fetchMetric(yf: any, symbol: string, isTarget: boolean, fxP: Promise<{ rate: number }>, base?: string): Promise<PeerMetric | null> {
  try {
    ...
    const pe = peRaw != null && peRaw > 0 ? +peRaw.toFixed(2) : null
    return { ticker: symbol.replace(/\.(KS|KQ)$/i, ''), name, industry, sameInd: false, peg, pegBaseEffect, opMargin, debtRatio, mcapUsd, psr, pe, isTarget }
  } catch { return null }
}
