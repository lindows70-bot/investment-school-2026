...
export default function ExitPlanBoard() {
  ...
  if (!data) return (
    <div style={{ margin: '14px 0', padding: '12px 16px', borderRadius: 12, background: TK.slate900, border: `1px solid ${TK.border}`, fontSize: FS.tiny, color: TK.sub2 }}>
      🚪 출구 플랜 계산 중…
    </div>
  )
  ...
  const Line = ({ icon, label, line, distPct, broken, mkt, tip }: {
    ...
    <span title={tip} style={{ display: 'inline-flex', alignItems: 'center', gap: 4, fontSize: FS.tiny, whiteSpace: 'nowrap' }}>
      <span>{icon}</span>
  ...
  return (
    <div style={{ margin: '14px 0', borderRadius: 12, background: TK.slate900, border: `1px solid ${TK.border}`, overflow: 'hidden' }}>
      <button onClick={() => setOpen(o => !o)} style={{ width: '100%', display: 'flex', alignItems: 'center', gap: 8, padding: '11px 16px', background: 'transparent', border: 'none', cursor: 'pointer', textAlign: 'left' }}>
        ...
        <b style={{ fontSize: FS.tiny, color: TK.slate100 }}>출구 플랜 — 보유 종목 매도 계획</b>
        {alerted.length > 0 && (
          <span style={{ fontSize: FS.tiny, fontWeight: 800, color: TK.amber400, background: '#78350f33', border: `1px solid ${TK.amber400}55`, borderRadius: 6, padding: '2px 7px' }}>
            매도 압력 신호 {alerted.length}종목
          </span>
        )}
        <span style={{ marginLeft: 'auto', fontSize: FS.tiny, color: TK.sub2 }}>{data.asOf} · {open ? '접기 ▴' : '펼치기 ▾'}</span>
      </button>

      {open && (
        <div style={{ padding: '0 12px 12px', display: 'flex', flexDirection: 'column', gap: 8 }}>
          <div style={{ fontSize: FS.tiny, color: TK.sub2, lineHeight: 1.6, padding: '0 4px' }}>
            ...
            return (
              <div key={it.ticker} style={{ borderRadius: 10, background: TK.slate950, border: `1px solid ${it.signals.length >= 2 ? `${TK.amber400}66` : TK.border}`, padding: '10px 12px' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
                  <b style={{ fontSize: FS.tiny, color: TK.slate100 }}>{flagOf(it.market, it.ticker)} {it.name}</b>
                  <span style={{ fontSize: FS.tiny, color: TK.sub, fontFamily: 'monospace', fontWeight: 700 }}>{it.ticker}</span>
                  <b style={{ fontSize: FS.tiny, color: pnlC, fontFamily: 'monospace' }}>{it.pnlPct >= 0 ? '+' : ''}{it.pnlPct}%</b>
                  <span style={{ fontSize: FS.tiny, color: lightC, border: `1px solid ${lightC}55`, borderRadius: 5, padding: '1px 6px' }}>
                    {it.light === 'green' ? '구조 양호' : it.light === 'red' ? '구조 붕괴' : it.defBroken ? '구조 약화' : '구조 중립'}
                  </span>
                  <span style={{ marginLeft: 'auto', fontSize: FS.tiny, color: TK.sub2, fontFamily: 'monospace' }}>종가 {fmtP(it.price, it.market)}</span>
                ...
                {it.signals.length > 0 && (
                  <div style={{ display: 'flex', gap: 5, flexWrap: 'wrap', marginTop: 7 }}>
                    {it.signals.map((s, i) => (
                      <span key={i} title={s.detail} style={{ fontSize: FS.tiny, fontWeight: 700, color: TK.amber400, background: '#78350f22', border: `1px solid ${TK.amber400}44`, borderRadius: 5, padding: '2px 6px', whiteSpace: 'nowrap' }}>
                        {s.icon} {s.label}
                      </span>
                    ))}
                    {it.fund === 'BUY' && (
                      <span title="Jarvis 펀더멘탈 진단은 매수 기회 — 기술 신호는 단기 경계 참고(펀더 멀쩡한 하락에 저점 매도 주의)"
                        style={{ fontSize: FS.tiny, fontWeight: 700, color: TK.green400, background: '#14532d22', border: `1px solid ${TK.green400}44`, borderRadius: 5, padding: '2px 6px' }}>
                        🟢 펀더 매수기회
                      </span>
                    )}
                  </div>
                ...
                {it.thesis && (
                  <div style={{ display: 'flex', alignItems: 'center', gap: 6, flexWrap: 'wrap', marginTop: 7, padding: '6px 8px', borderRadius: 7, background: TK.slate900, border: `1px dashed ${TK.border}` }}>
                    <span title="매수 순간 자동 저장된 신호(PEG·이익·수급·계절)와 지금을 대조 — 린치: '산 이유가 사라지면 판다'. 손절을 감정이 아니라 근거 소멸로 판단하는 훈련"
                      style={{ fontSize: FS.tiny, fontWeight: 800, color: TK.slate300 }}>
                    ...
                    {it.thesis.axes.map((a, i) => {
                      ...
                      return (
                        <span key={i} style={{ fontSize: FS.tiny, color: c, border: `1px solid ${c}44`, borderRadius: 5, padding: '1px 6px', whiteSpace: 'nowrap' }}>
                          {a.icon} {a.label} {a.then} → {a.now}
                        </span>
                      )
                    })}
                    <b style={{ marginLeft: 'auto', fontSize: FS.tiny,
                ...
                {it.buffett && it.buffett.level !== 'na' && (
                  <div style={{ display: 'flex', alignItems: 'center', gap: 6, flexWrap: 'wrap', marginTop: 7, padding: '6px 8px', borderRadius: 7, background: TK.slate900, border: `1px dashed ${it.buffett.level === 'strong' ? TK.red400 + '66' : TK.border}` }}>
                    ...
                    {it.buffett.checks.map(c => {
                      ...
                      return (
                        <span key={c.key} title={c.detail} style={{ fontSize: FS.tiny, color: col, border: `1px solid ${col}44`, borderRadius: 5, padding: '1px 6px', whiteSpace: 'nowrap' }}>
                          {c.icon} {c.label} {c.hit == null ? '—' : c.hit ? '훼손' : '정상'}
                        </span>
                      )
                    })}
                    <b style={{ marginLeft: 'auto', fontSize: FS.tiny, color: it.buffett.level === 'strong' ? TK.red400 : it.buffett.level === 'watch' ? TK.amber400 : TK.green400 }}>
                      {it.buffett.headline}
                    </b>
                  </div>
                )}

                <div style={{ marginTop: 7, fontSize: FS.tiny, color: TK.slate300, lineHeight: 1.55 }}>
          ...
          <div style={{ fontSize: FS.tiny, color: TK.sub2, lineHeight: 1.6, padding: '2px 4px' }}>
