    ...
    <div style={{ background: CARD, borderRadius: 12, border: `1px solid ${BORDER}`, padding: '14px 16px' }}>
      ...
      {current && (
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap',
          ...
          <span style={{ color: TK.slate200, fontWeight: 800, fontSize: 13 }}>현재 구간: {current.label}</span>
          <span style={{ color: TK.sub5, fontSize: 11 }}>{valuationText}</span>
        </div>
      )}
      ...
      {eduOpen && (
        <div style={{ color: TK.sub5, fontSize: 11, lineHeight: 1.65, marginTop: 6, display: 'flex', flexDirection: 'column', gap: 8 }}>
          <div>📐 <b style={{ color: TK.blue300 }}>원리</b> — 비트코인의 장기 가격을 <b>로그 회귀선</b>(시간의 로그에 비례해 우상향하는 추세선)에 맞추고, 그 위아래로 표준편차만큼 색띠를 두른 것입니다. 가격이 <b style={{ color: '#2f7fd1' }}>아래쪽 파란 띠</b>에 있으면 역사적으로 저평가(축적 구간), <b style={{ color: '#e0382f' }}>위쪽 빨간 띠</b>면 과열(버블 경계)이었습니다.</div>
          <div>🧭 <b style={{ color: TK.btcOrange }}>읽는 법</b> — 정밀한 매수·매도 신호가 아니라 &lsquo;지금 사이클에서 대략 어디쯤인가&rsquo;를 보는 <b>심리 온도계</b>입니다. 반감기 사이클(패널 ①)·메이어 멀티플과 함께 보세요.</div>
          <div style={{ color: TK.sub }}>⚠️ <b>정직한 한계</b> — 회귀선은 <b>과거 추세의 외삽</b>이라 미래를 보장하지 않습니다. 적합에 쓰인 데이터 구간(여기선 Yahoo 10년 주봉)에 따라 띠 위치가 달라지며, 시장 구조가 바뀌면(채택 성숙·변동성 감소) 기울기도 변합니다. 교육용 참고 지표일 뿐입니다.</div>
        </div>
      )}
    </div>
  )
