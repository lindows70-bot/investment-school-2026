...
const BANDS = [
  { from: 0, to: 12, name: '침체기 (Bear)', short: '침체기', color: '#60a5fa', years: '2014 · 2018 · 2022 · 2026', desc: '고점 후 깊은 조정 — 과거 −70%대 드로다운' },
  { from: 12, to: 24, name: '상승 준비기 (Pre-Bull)', short: '준비기 · 승부구간', color: '#a78bfa', years: '2015 · 2019 · 2023 · 2027', desc: '바닥 다지기·축적 — 원본 포스터의 "승부구간"' },
...
const LINES = [
  ...
  { key: 'c2022', name: '2022~25 사이클', color: '#2dd4bf', dash: '5 3', w: 1.4 },
  { key: 'c2026', name: '2026~ (현재)', color: '#f1f5f9', dash: undefined, w: 2.6 },
]
  ...
  return (
    <div style={{ background: CARD, border: `1px solid ${BORDER}`, borderRadius: 14, padding: '16px 18px' }}>
      ...
      <div style={{ background: `${cur.color}10`, border: `1px solid ${cur.color}44`, borderRadius: 9, padding: '8px 12px', marginBottom: 12, fontSize: 11.5, lineHeight: 1.6 }}>
        ...
        <span style={{ color: '#aab6c4' }}> — 과거 3번의 사이클에서 이 자리는 {cur.desc.split(' — ')[0]} 구간이었습니다. 각본이 반복된다는 보장은 없지만, 지금의 하락을 &lsquo;공포&rsquo;가 아니라 &lsquo;사이클의 자리&rsquo;로 읽는 훈련이 목적입니다.</span>
      </div>
      ...
      <ResponsiveContainer width="100%" height={320}>
        <LineChart data={nav.overlay} margin={{ top: 20, right: 14, left: 6, bottom: 0 }}>
          ...
          <ReferenceLine x={nav.mNow} stroke="#f1f5f9" strokeDasharray="5 3"
            label={{ value: '◀ 현재', position: 'insideTopRight', fill: '#f1f5f9', fontSize: 10 }} />
          {LINES.map(l => (
