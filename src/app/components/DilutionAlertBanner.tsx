...
export default function DilutionAlertBanner() {
  ...
  return (
    <div style={{ background: `linear-gradient(135deg,${TK.orange400}14,${TK.bg1})`, border: `1px solid ${TK.orange400}55`, borderRadius: 12, padding: '10px 14px', display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
      <span style={{ fontSize: FS.tiny, fontWeight: 800, color: TK.orange400 }}>🚨 희석 경보</span>
      <span style={{ fontSize: FS.tiny, color: TK.sub4 }}>보유 종목 DART 공시(180일) — 유상증자·주식관련사채·감자</span>
      {data.alerts.slice(0, 6).map((a, i) => {
        ...
        return (
          <a key={a.rcpNo + i} href={`https://dart.fss.or.kr/dsaf001/main.do?rcpNo=${a.rcpNo}`} target="_blank" rel="noreferrer"
            ...
            style={{ display: 'inline-flex', alignItems: 'center', gap: 5, background: '#7f1d1d22', border: `1px solid ${m.color}55`, borderRadius: 7, padding: '3px 9px', fontSize: FS.tiny, whiteSpace: 'nowrap', textDecoration: 'none' }}>
            {a.recent && <b style={{ color: TK.orange400, fontSize: FS.tiny }}>NEW</b>}
            <b style={{ color: m.color, fontSize: FS.tiny }}>{a.type}</b>
            <b style={{ color: TK.slate200 }}>{a.name}</b>
            <span style={{ color: TK.sub2, fontSize: FS.tiny, fontFamily: 'monospace' }}>{a.date.slice(5)}</span>
            <span style={{ color: TK.sub4, fontSize: FS.tiny }}>원문↗</span>
          </a>
        )
      })}
      <span style={{ fontSize: FS.tiny, color: TK.sub2 }}>{hasRecent ? '30일 내 공시 포함 — ' : ''}매도 지시 아님 · 용도·규모는 원문 확인</span>
      <button onClick={() => setDismissed(true)} style={{ marginLeft: 'auto', background: 'transparent', border: 'none', color: TK.sub, cursor: 'pointer', fontSize: FS.body }}>✕</button>
    </div>
  )
}
