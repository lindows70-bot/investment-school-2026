...
const SECTOR_COLORS = [TK.blue400, TK.green400, TK.amber400, TK.orange400, TK.violet400, TK.cyan400, TK.red400, TK.slate400, TK.emerald400, '#e879f9', '#facc15', TK.slate500]
...
const axisColor = (s: AxisStatus) => s === 'pass' ? TK.green400 : s === 'fail' ? TK.red400 : TK.slate500
...
const posLabel = (p: number) => p <= 25 ? { t: '바닥권', c: TK.green400 } : p <= 50 ? { t: '중하단', c: TK.lime400 } : p <= 75 ? { t: '중상단', c: TK.amber400 } : p <= 92 ? { t: '고점권', c: TK.orange400 } : { t: '신고가권', c: TK.red400 }
  ...
  return (
    <span title={`52주 밴드 ${posPct}% — ${hot ? '신고가권: 고점 추격 매수 주의' : '고점권: 추격 주의'}`}
      style={{ display: 'inline-flex', alignItems: 'center', gap: 2, borderRadius: 5, padding: '0px 5px', fontSize: 9, fontWeight: 800, whiteSpace: 'nowrap',
        background: hot ? 'rgba(248,113,113,0.16)' : 'rgba(251,146,60,0.12)', color: hot ? TK.red400 : TK.orange400,
        border: `1px solid ${hot ? `${TK.red400}66` : `${TK.orange400}55`}` }}>
      🔺{hot ? '신고가 추격주의' : '고점'}
    </span>
  )
}
...
function Spark({ ctx }: { ctx: PriceContext }) {
  ...
  const col = up ? TK.green400 : TK.red400
  return (
...
function Band52({ ctx }: { ctx: PriceContext }) {
  ...
  return (
    <span style={{ display: 'inline-flex', alignItems: 'center', gap: 6, flexShrink: 0 }} title={`52주 ${ctx.low52.toLocaleString()} ~ ${ctx.high52.toLocaleString()} · 현재 ${ctx.price.toLocaleString()}`}>
      <span style={{ position: 'relative', width: 72, height: 6, borderRadius: 3, background: `linear-gradient(90deg,${TK.green500}44,${TK.amber400}44,${TK.red500}44)` }}>
        <span style={{ position: 'absolute', left: `calc(${ctx.posPct}% - 3px)`, top: -2, width: 6, height: 10, borderRadius: 2, background: lb.c, border: `1px solid ${TK.bg3}` }} />
      </span>
...
export default function QuantBuilderLab() {
  ...
  if (loading) return <div style={{ background: CARD, borderRadius: 12, padding: 24, border: `1px solid ${BORDER}`, color: TK.sub, fontSize: 12 }}>🛰️ 퀀트 빌더가 시장 국면과 3축 데이터를 수집 중…(최초 1회 최대 1분)</div>
  if (!d) return <div style={{ background: CARD, borderRadius: 12, padding: 24, border: `1px solid ${BORDER}`, color: TK.sub, fontSize: 12 }}>설계 데이터를 불러오지 못했습니다 — 잠시 후 새로고침해주세요.</div>
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
      ...
      <div style={{ background: 'linear-gradient(135deg,rgba(34,211,238,0.10),rgba(167,139,250,0.07))', border: '1px solid rgba(34,211,238,0.35)', borderRadius: 12, padding: '14px 18px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap' }}>
          ...
          <span style={{ color: TK.slate200, fontWeight: 800, fontSize: 16 }}>AI 1억 백지 퀀트 빌더</span>
          <span style={{ color: TK.sub, fontSize: 12 }}>코어-새틀라이트 — 백지에서 시작하는 국면 맞춤 설계</span>
          <span style={{ marginLeft: 'auto', display: 'inline-flex', alignItems: 'center', gap: 6 }}>
            <span style={{ color: TK.sub, fontSize: 11.5 }}>총투자금</span>
            <input value={amountEok} onChange={e => setAmountEok(e.target.value.replace(/[^0-9.]/g, ''))}
              style={{ width: 64, background: TK.bg3, border: `1px solid ${BORDER}`, borderRadius: 7, padding: '5px 9px', color: TK.slate200, fontSize: 13, fontWeight: 800, fontFamily: 'monospace', textAlign: 'right' }} />
            <span style={{ color: TK.slate200, fontSize: 12.5, fontWeight: 700 }}>억원</span>
          </span>
        </div>
        <div style={{ color: TK.sub5, fontSize: 12, lineHeight: 1.65, marginTop: 7 }}>
          현재 국면 <b style={{ color: TK.cyan400 }}>{d.seasonLabel}</b> → Core(시장 ETF) <b style={{ color: TK.cyan400 }}>{d.coreRatio}%</b> + Satellite(최정예 개별주) <b style={{ color: TK.violet400 }}>{d.satelliteRatio}%</b>. {d.rationale}
        </div>
      </div>
      ...
      <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap' }}>
        ...
        <div style={{ flex: '1 1 380px', minWidth: 320, background: CARD, borderRadius: 12, border: `1px solid ${BORDER}`, padding: '14px 16px' }}>
          <div style={{ color: TK.slate200, fontWeight: 800, fontSize: 13, marginBottom: 4 }}>🪐 코어 행성 × 위성 궤도</div>
          <div style={{ color: TK.sub, fontSize: 10.5, marginBottom: 6 }}>중심 = 시장 ETF(묵직한 본체) · 궤도 = 알파를 노리는 정예 개별주</div>
          <div style={{ position: 'relative', height: 340, overflow: 'hidden' }}>
            ...
            <div style={{ position: 'absolute', left: '50%', top: '50%', transform: 'translate(-50%,-50%)', width: 150, height: 150, borderRadius: '50%', background: 'radial-gradient(circle at 35% 30%, rgba(34,211,238,0.35), rgba(34,211,238,0.08) 60%, rgba(15,17,23,0.9))', border: '1.5px solid rgba(34,211,238,0.5)', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 1 }}>
              <span style={{ color: TK.cyan400, fontWeight: 900, fontSize: 15 }}>CORE {d.coreRatio}%</span>
              <span style={{ color: TK.slate300, fontSize: 10.5, fontFamily: 'monospace' }}>₩{fmtWon(amountKrw * d.coreRatio / 100)}</span>
              {d.core.map(c => (
                <span key={c.ticker} style={{ color: TK.sub8, fontSize: 9.5 }}>{c.ticker} {c.weightPct}%</span>
              ))}
            </div>
            {/* 위성들 */}
            {satPos.map(({ s, x, y }) => (
              <button key={s.ticker} onClick={() => setOpenSat(o => o === s.ticker ? null : s.ticker)}
                ...
                style={{ position: 'absolute', left: `calc(50% + ${x}px)`, top: `calc(50% + ${y}px)`, transform: 'translate(-50%,-50%)', width: 64, height: 64, borderRadius: '50%', cursor: 'pointer',
                  ...
                  border: `1.5px solid ${openSat === s.ticker ? TK.slate200 : s.passCount === 3 ? `${TK.green400}88` : `${TK.violet400}88`}`,
                  display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: 0 }}>
                <span style={{ color: TK.slate200, fontWeight: 800, fontSize: 9.5, lineHeight: 1.15, maxWidth: 58, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{s.market === 'KR' ? s.name.slice(0, 5) : s.ticker}</span>
                <span style={{ color: TK.sub8, fontSize: 8.5, fontFamily: 'monospace' }}>{s.weightPct}%</span>
              </button>
            ))}
            {d.satellites.length === 0 && (
              <div style={{ position: 'absolute', left: '50%', top: '88%', transform: 'translate(-50%,-50%)', color: TK.sub, fontSize: 11 }}>3축을 모두 통과한 위성이 아직 없습니다(데이터 수집 중)</div>
            )}
          </div>
          <div style={{ color: TK.sub, fontSize: 9.5, display: 'flex', gap: 12 }}>
            <span><span style={{ color: TK.green400 }}>●</span> 3축 전부 통과(최정예)</span>
            <span><span style={{ color: TK.violet400 }}>●</span> 2축 통과+무탈락(정예)</span>
            <span>위성 클릭 → 상세</span>
          </div>
        </div>
        ...
        <div style={{ flex: '1 1 320px', minWidth: 280, background: CARD, borderRadius: 12, border: `1px solid ${BORDER}`, padding: '14px 16px' }}>
          <div style={{ color: TK.slate200, fontWeight: 800, fontSize: 13, marginBottom: 4 }}>🔬 ETF 투시경 — {fmtWon(amountKrw)}원의 실질 섹터</div>
          <div style={{ color: TK.sub, fontSize: 10.5, marginBottom: 4 }}>Core ETF를 분해해 위성과 합산한 진짜 노출 — ETF 뒤에 숨은 쏠림까지 봅니다</div>
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: '3px 10px', marginTop: 2 }}>
            {d.effectiveSectors.slice(0, 8).map((s, i) => (
              <span key={s.sector} style={{ fontSize: 10, color: TK.sub5 }}>
      ...
      <div style={{ background: CARD, borderRadius: 12, border: `1px solid ${BORDER}`, padding: '14px 16px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8, flexWrap: 'wrap' }}>
          <span style={{ color: TK.slate200, fontWeight: 800, fontSize: 13 }}>📋 처방전 — {fmtWon(amountKrw)}원 배분</span>
          <span style={{ color: TK.sub, fontSize: 10.5 }}>{d.axisRule}</span>
        </div>
        {/* Core */}
        {d.core.map(c => (
          <div key={c.ticker} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '6px 0', borderTop: `1px solid ${BORDER}`, fontSize: 11.5, flexWrap: 'wrap' }}>
            ...
            <span style={{ marginLeft: c.priceCtx ? 0 : 'auto', color: TK.cyan400, fontWeight: 800, fontFamily: 'monospace' }}>{c.weightPct}%</span>
            <span style={{ color: TK.slate300, fontFamily: 'monospace', minWidth: 76, textAlign: 'right' }}>₩{fmtWon(amountKrw * c.weightPct / 100)}</span>
          </div>
        ))}
        {/* Satellites */}
        {d.satellites.map(s => (
          <div key={s.ticker}>
            <div onClick={() => setOpenSat(o => o === s.ticker ? null : s.ticker)} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '6px 0', borderTop: `1px solid ${BORDER}`, fontSize: 11.5, flexWrap: 'wrap', cursor: 'pointer' }}>
              ...
              <span style={{ color: TK.sub, fontSize: 10 }}>통합 {s.combined}</span>
              {s.priceCtx && (
                <span style={{ marginLeft: 'auto', display: 'inline-flex', alignItems: 'center', gap: 10 }}>
              ...
              <span style={{ marginLeft: s.priceCtx ? 0 : 'auto', color: TK.violet400, fontWeight: 800, fontFamily: 'monospace' }}>{s.weightPct}%</span>
              <span style={{ color: TK.slate300, fontFamily: 'monospace', minWidth: 76, textAlign: 'right' }}>₩{fmtWon(amountKrw * s.weightPct / 100)}</span>
            </div>
            {openSat === s.ticker && (
              <div style={{ margin: '2px 0 6px', background: TK.bg3, border: `1px solid ${BORDER}`, borderRadius: 8, padding: '8px 12px', fontSize: 10.5, color: TK.sub5, lineHeight: 1.7 }}>
                {AXIS_META.map(a => (
                ...
                <span style={{ marginRight: 14 }} title="주가매출비율(P/S) — 적자기업·고성장주는 PER/PEG가 무의미할 때 '매출 대비 밸류'를 본다. 동종 업종끼리 비교해야 의미가 있다(산업마다 정상치 다름).">
                  💵 매출배수: <b style={{ color: s.psr != null ? TK.slate300 : TK.sub }}>{s.psr != null ? `PSR ${s.psr.toFixed(1)}배` : 'PSR 자료없음'}</b>
                </span>
                {s.badges.length > 0 && <div style={{ marginTop: 3 }}>{s.badges.map(b => <span key={b} style={{ marginRight: 8, color: TK.sub }}>{b}</span>)}</div>}
