...
export default function LynchAutoPanel(p: LynchAutoProps) {
  ...
  const Box = ({ title, children }: { title: string; children: React.ReactNode }) => (
    <div style={{ flex: '1 1 220px', minWidth: 200, background: TK.bg3, borderRadius: 10, border: `1px solid ${BORDER}`, padding: '11px 13px' }}>
      <div style={{ color: TK.sub, fontSize: 10.5, fontWeight: 700, marginBottom: 6 }}>{title}</div>
      {children}
    </div>
  )
  ...
  return (
    <div style={{ background: CARD, borderRadius: 14, border: `1px solid ${BORDER}`, padding: '16px 18px', display: 'flex', flexDirection: 'column', gap: 13 }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
        ...
        <span style={{ color: TK.slate200, fontWeight: 800, fontSize: 14 }}>피터린치 자동 분석</span>
        <span style={{ color: TK.sub2, fontSize: 11 }}>{p.name} · 6대 분류 × PEG × 이익선</span>
      </div>

      <div style={{ display: 'flex', gap: 11, flexWrap: 'wrap' }}>
        <Box title="🏷️ 6대 분류">
          <div style={{ color: TK.violet400, fontWeight: 800, fontSize: 15 }}>{catKr}</div>
          <div style={{ color: TK.sub13, fontSize: 10.5, marginTop: 3 }}>{cat === 'fast_grower' ? '성장률 = 적정 PER (린치 공식)' : cat === 'cyclical' ? '이익 정점 = PER 최저 함정 주의' : cat === 'stalwart' ? '꾸준한 대형 우량주' : '분류별 가치 기준 적용'}</div>
        </Box>
        <Box title={`💎 PEG 해석 ${peg != null ? `(${peg.toFixed(2)})` : ''}`}>
          <div style={{ color: pegView.color, fontWeight: 800, fontSize: 14 }}>{pegView.label}</div>
          <div style={{ color: TK.sub13, fontSize: 10.5, marginTop: 3, lineHeight: 1.5 }}>{pegView.desc}</div>
        </Box>
