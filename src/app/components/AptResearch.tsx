...
export default function AptResearch({ initialLawd }: { initialLawd?: string } = {}) {
  ...
  const v = sel?.value
  const ratioTone = v?.jeonseRatio == null ? TK.slate200 : v.jeonseRatio >= 70 ? TK.green400 : v.jeonseRatio >= 55 ? TK.yellow500 : TK.red400
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, alignItems: 'center' }}>
        <select value={lawd} onChange={e => { setLawd(e.target.value); setApt(''); setArea(null) }}
          style={{ background: TK.bg3, border: `1px solid ${BORDER}`, borderRadius: 9, padding: '9px 12px', color: TK.slate200, fontSize: 13 }}>
        ...
        <form onSubmit={e => { e.preventDefault(); const q = aptInput.trim(); if (q) { setApt(q); setArea(null) } }} style={{ display: 'flex', gap: 6 }}>
          <input value={aptInput} onChange={e => setAptInput(e.target.value)} placeholder="단지 검색 (예: 오금동 대림)"
            style={{ background: TK.bg3, border: `1px solid ${BORDER}`, borderRadius: 9, padding: '9px 12px', color: TK.slate200, fontSize: 13, width: 190 }} />
          <button type="submit" style={{ background: TK.orange400, border: 'none', borderRadius: 9, padding: '9px 14px', color: TK.bg1, fontSize: 12.5, fontWeight: 800, cursor: 'pointer' }}>🔍 검색</button>
        </form>
        <span style={{ color: TK.sub, fontSize: 11 }}>최근 24개월 실거래(국토부 신고 기준·호가 아님) · 첫 로드는 수집에 ~30초</span>
      </div>

      {loading ? (
        <div style={{ background: CARD, borderRadius: 12, padding: 24, border: `1px solid ${BORDER}`, color: TK.sub, fontSize: 12 }}>🔍 {regionLabel?.name ?? lawd} 실거래 24개월을 수집 중…</div>
      ) : err ? (
        ...
        <>
          ...
          <div style={{ background: CARD, border: `1px solid ${BORDER}`, borderRadius: 14, padding: '14px 18px' }}>
            <div style={{ color: TK.slate200, fontWeight: 800, fontSize: 13, marginBottom: 8 }}>
              🏢 {d.sido} {regionLabel?.name} — 거래 활발 단지 Top {d.complexes.length} <span style={{ color: TK.sub, fontWeight: 400, fontSize: 11 }}>(24개월 매매 건수순 · 클릭 선택)</span>
            </div>
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
              {d.complexes.map(c => (
                <button key={c.name} onClick={() => { setApt(c.name); setArea(null) }} style={{
                  ...
                  background: sel?.name === c.name ? TK.orange400 : TK.bg3, color: sel?.name === c.name ? TK.bg1 : TK.slate300,
                  border: `1px solid ${sel?.name === c.name ? TK.orange400 : BORDER}`,
              ...
              {sel && !d.complexes.some(c => c.name === sel.name) && (
                <button style={{
                  ...
                  background: TK.orange400, color: TK.bg1, border: `1px solid ${TK.orange400}`,
          ...
          {sel && (
            <>
                  ...
                  <span style={{ display: 'inline-flex', gap: 5 }}>
                    {sel.areas.map(a => (
                      <button key={a.area} onClick={() => setArea(a.area)} style={{
                        ...
                        background: sel.area === a.area ? TK.teal400 : TK.bg3, color: sel.area === a.area ? TK.bg1 : TK.sub,
                        border: `1px solid ${sel.area === a.area ? TK.teal400 : BORDER}`,
                ...
                {sel.overview && (
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginTop: 9 }}>
                    {([
                      ...
                      <span key={label} style={{ background: TK.bg3, border: `1px solid ${BORDER}`, borderRadius: 7, padding: '4px 10px', fontSize: 11, color: TK.slate300 }}>
                        <span style={{ color: TK.sub, fontWeight: 700 }}>{label}</span> <span style={{ fontWeight: 800 }}>{val}</span>
                      </span>
                    ))}
                    <span style={{ fontSize: 9.5, color: TK.sub, alignSelf: 'center' }}>서울시 공동주택 마스터</span>
                  </div>
                )}
                {v && (
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: 10, marginTop: 12 }}>
                    <div style={{ flex: '1 1 160px', background: TK.bg3, border: `1px solid ${BORDER}`, borderRadius: 10, padding: '10px 13px' }}>
                      <div style={{ color: TK.sub, fontSize: 10.5, fontWeight: 700 }}>💎 전세가율(사용가치 비율)</div>
                      <div style={{ color: ratioTone, fontSize: 19, fontWeight: 900, fontFamily: 'monospace' }}>{v.jeonseRatio != null ? `${v.jeonseRatio}%` : '—'}</div>
                      <div style={{ color: TK.sub, fontSize: 10 }}>매매 중위 {v.saleMed6 ?? '—'}억 vs 전세 중위 {v.jeonseMed6 ?? '—'}억 (6개월)</div>
                    </div>
                    <div style={{ flex: '1 1 160px', background: TK.bg3, border: `1px solid ${BORDER}`, borderRadius: 10, padding: '10px 13px' }}>
                      <div style={{ color: TK.sub, fontSize: 10.5, fontWeight: 700 }}>⛰️ 고점 대비</div>
                      <div style={{ color: v.vsPeak == null ? TK.slate200 : v.vsPeak >= -5 ? TK.red400 : v.vsPeak >= -20 ? TK.yellow500 : TK.green400, fontSize: 19, fontWeight: 900, fontFamily: 'monospace' }}>
                        {v.vsPeak != null ? `${v.vsPeak > 0 ? '+' : ''}${v.vsPeak}%` : '—'}
                      </div>
                      <div style={{ color: TK.sub, fontSize: 10 }}>역대 최고 실거래 {v.peak ?? '—'}억 대비 현재 중위</div>
                    </div>
                    <div style={{ flex: '1 1 160px', background: TK.bg3, border: `1px solid ${BORDER}`, borderRadius: 10, padding: '10px 13px' }}>
                      <div style={{ color: TK.sub, fontSize: 10.5, fontWeight: 700 }}>🐝 지역 국면 ({d.sido})</div>
              ...
              {/* 실거래 추이 차트 */}
              <div style={{ background: CARD, border: `1px solid ${BORDER}`, borderRadius: 14, padding: '16px 18px' }}>
                <div style={{ color: TK.slate200, fontWeight: 800, fontSize: 13 }}>📈 실거래 추이 — {sel.name} 전용 {sel.area}㎡(±2㎡)</div>
                <div style={{ color: TK.sub, fontSize: 11, margin: '3px 0 8px' }}>점 = 개별 실거래(빨강 매매·초록 전세) · 선 = 월 중위가. 전세와 매매의 간격이 좁을수록 &lsquo;사용가치가 받치는 가격&rsquo;.</div>
                <ResponsiveContainer width="100%" height={300}>
                  <ComposedChart data={chart} margin={{ top: 6, right: 8, left: 0, bottom: 0 }}>
                    <CartesianGrid stroke={TK.grid} strokeDasharray="3 3" />
                    <XAxis dataKey="ym" tick={{ fill: TK.sub, fontSize: 9.5 }} interval={2} allowDuplicatedCategory={false} />
                    <YAxis tick={{ fill: TK.sub, fontSize: 10 }} width={44} domain={['auto', 'auto']} tickFormatter={vv => `${vv}억`} />
                    ...
                    <Scatter data={dots} dataKey="saleDot" name="매매 실거래" fill={TK.red400} fillOpacity={0.55} shape="circle" tooltipType="none" />
                    <Scatter data={dots} dataKey="jeonseDot" name="전세 실거래" fill={TK.green400} fillOpacity={0.5} shape="circle" tooltipType="none" />
                    <Line type="monotone" dataKey="sale" name="매매 월 중위" stroke={TK.slate100} strokeWidth={2} dot={false} connectNulls />
                    <Line type="monotone" dataKey="jeonse" name="전세 월 중위" stroke={TK.teal400} strokeWidth={1.6} dot={false} connectNulls />
                  </ComposedChart>
                </ResponsiveContainer>
              </div>
              ...
              {/* 최근 거래 테이블 */}
              <div style={{ background: CARD, border: `1px solid ${BORDER}`, borderRadius: 14, padding: '14px 18px' }}>
                <div style={{ color: TK.slate200, fontWeight: 800, fontSize: 13, marginBottom: 8 }}>🧾 최근 실거래 (신고 기준)</div>
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(215px,1fr))', gap: 6 }}>
                  {sel.deals.slice(0, 24).map((dl, i) => (
                    <div key={i} style={{ background: TK.bg3, border: `1px solid ${BORDER}`, borderRadius: 8, padding: '7px 11px', fontSize: 11 }}>
                      <span style={{ color: dl.type === '매매' ? TK.red400 : TK.green400, fontWeight: 800 }}>{dl.type}</span>
                      <span style={{ color: TK.slate200, fontWeight: 800, fontFamily: 'monospace', marginLeft: 7 }}>{dl.price}억</span>
                      <span style={{ color: TK.sub, marginLeft: 7 }}>{dl.ym}-{String(dl.day).padStart(2, '0')} · {dl.area}㎡ · {dl.floor ?? '—'}층</span>
                    </div>
                  ))}
                </div>
              </div>
