  ...
  return (
    <div style={{ background: 'linear-gradient(135deg,rgba(249,115,22,0.10),rgba(239,68,68,0.05))', border: '1px solid rgba(249,115,22,0.35)', borderRadius: 12, padding: '12px 16px' }}>
      ...
      <button onClick={() => setCollapsed(v => !v)}
        ...
        <span style={{ color: TK.orange400, fontWeight: 800, fontSize: 13.5 }}>오늘 시장의 눈 — 마켓 카탈리스트</span>
        {collapsed
          ? <span style={{ color: TK.sub5, fontSize: 11, flex: 1, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{d.catalysts[0]?.title ?? d.marketMood ?? ''}</span>
          : (d.marketMood && <span style={{ color: TK.sub5, fontSize: 11 }}>{d.marketMood}</span>)}
        <span style={{ marginLeft: 'auto', color: TK.orange400, fontSize: 11, fontWeight: 700, whiteSpace: 'nowrap' }}>{collapsed ? `▼ 펼치기${d.catalysts.length ? ` (${d.catalysts.length})` : ''}` : '▲ 접기'}</span>
      </button>
      ...
      {d.catalysts.map((c, i) => (
        <div key={i} style={{ borderTop: i > 0 ? `1px solid ${BORDER}` : 'none' }}>
          <button onClick={() => setOpen(o => o === i ? null : i)}
            ...
            <span style={{ color: TK.amber400, fontWeight: 800, fontSize: 11 }}>{['①', '②', '③'][i]}</span>
            <span style={{ color: TK.slate200, fontWeight: 700, fontSize: 12.5, flex: 1 }}>{c.title}</span>
            {c.tickers.slice(0, 4).map(t => (
              <span key={t} style={{ background: TK.bg3, border: `1px solid ${BORDER}`, borderRadius: 5, padding: '1px 7px', color: TK.blue300, fontSize: 9.5, fontWeight: 700 }}>{t}</span>
            ))}
            <span style={{ color: TK.sub, fontSize: 10 }}>{open === i ? '▲' : '▼'}</span>
          </button>
          {open === i && (
            <div style={{ padding: '0 0 9px 22px' }}>
              <div style={{ color: TK.slate300, fontSize: 11.5, lineHeight: 1.65 }}>{c.why}</div>
              <div style={{ marginTop: 5, background: 'rgba(245,158,11,0.07)', border: '1px solid rgba(245,158,11,0.3)', borderRadius: 8, padding: '7px 11px', color: TK.sub15, fontSize: 11, lineHeight: 1.65 }}>
                🤖 <b style={{ color: TK.amber400 }}>자비스 한줄 처방</b> — {c.jarvisTip}
              </div>
            </div>
          )}
        </div>
