...
const RISK: Record<StableRisk, { c: string; ko: string }> = {
  low: { c: TK.green500, ko: '상대적 안전' }, mid: { c: TK.amber400, ko: '중위험' }, high: { c: TK.red500, ko: '초고위험' },
}
...
export default function StablecoinRadar() {
  ...
  if (loading) return <div style={{ background: CARD, borderRadius: 12, border: `1px solid ${BORDER}`, padding: 20, color: TK.sub, fontSize: 12 }}>💵 스테이블코인 시장을 분석하는 중…</div>
  if (!d) return <div style={{ background: CARD, borderRadius: 12, border: `1px solid ${BORDER}`, padding: 20, color: TK.sub, fontSize: 12 }}>스테이블코인 데이터를 불러오지 못했습니다.</div>
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
      ...
      <div style={{ background: 'linear-gradient(135deg,rgba(38,161,123,0.1),rgba(96,165,250,0.05))', border: '1px solid rgba(38,161,123,0.3)', borderRadius: 12, padding: '14px 17px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 9, flexWrap: 'wrap' }}>
          ...
          <span style={{ color: TK.slate200, fontWeight: 800, fontSize: 15 }}>스테이블코인 레이더</span>
          <span style={{ color: TK.sub, fontSize: 11.5 }}>암호시장의 디지털 달러·유동성</span>
        </div>
        <div style={{ color: TK.sub5, fontSize: 12, lineHeight: 1.6, marginTop: 5 }}>
          비트코인이 <b style={{ color: TK.btcOrange }}>디지털 금</b>이면, 스테이블코인은 <b style={{ color: '#26a17b' }}>디지털 달러</b> — 암호시장의 현금·유동성입니다. <b>시총이 늘면 살 돈이 대기 중(강세 연료)</b>, 줄면 자금 이탈, <b>페그가 깨지면 시스템 위기</b>(테라 사태). &ldquo;차트만 보지 말고 들어온 달러를 봐라.&rdquo;
        </div>
      </div>
      ...
      <div style={{ background: CARD, borderRadius: 12, border: `1px solid ${BORDER}`, padding: '14px 16px' }}>
        <div style={{ display: 'flex', alignItems: 'baseline', gap: 8, flexWrap: 'wrap', marginBottom: 3 }}>
          <span style={{ color: TK.slate200, fontWeight: 800, fontSize: 13.5 }}>💧 시장 유동성 게이지</span>
          <span style={{ color: TK.sub, fontSize: 11 }}>전체 USD 스테이블코인 시총(매수 대기 자금)</span>
          <span style={{ marginLeft: 'auto', color: '#26a17b', fontWeight: 800, fontSize: 14 }}>{fmtB(d.totalMcap)}</span>
          {d.mcapChange30d != null && <span style={{ color: liqUp ? TK.green400 : TK.red400, fontWeight: 700, fontSize: 12 }}>30일 {liqUp ? '▲' : '▼'} {d.mcapChange30d}%</span>}
        </div>
      ...
      {d.dominance != null && (() => {
        ...
        const zc = z === 'high' ? TK.green500 : z === 'low' ? TK.red500 : TK.amber400
        ...
        return (
          <div style={{ background: CARD, borderRadius: 12, border: `1px solid ${BORDER}`, padding: '14px 16px' }}>
            <div style={{ display: 'flex', alignItems: 'baseline', gap: 8, flexWrap: 'wrap', marginBottom: 8 }}>
              <span style={{ color: TK.slate200, fontWeight: 800, fontSize: 13.5 }}>🪙 스테이블코인 도미넌스</span>
              <span style={{ color: TK.sub, fontSize: 11 }}>= 스테이블 시총 ÷ 전체 암호 시총 = 코인판 &lsquo;현금 비중&rsquo;</span>
      ...
      {d.chains && d.chains.length > 0 && (() => {
        const CHC = ['#627eea', TK.red500, '#14f195', '#f3ba2f', '#06b6d4', TK.violet400, TK.slate500]
        return (
          <div style={{ background: CARD, borderRadius: 12, border: `1px solid ${BORDER}`, padding: '14px 16px' }}>
            <div style={{ display: 'flex', alignItems: 'baseline', gap: 8, marginBottom: 3, flexWrap: 'wrap' }}>
              <span style={{ color: TK.slate200, fontWeight: 800, fontSize: 13.5 }}>⛓️ 체인별 분포</span>
              <span style={{ color: TK.sub, fontSize: 11 }}>스테이블코인(달러)이 어느 생태계에 있나</span>
            </div>
            <div style={{ color: TK.sub2, fontSize: 11, marginBottom: 9, lineHeight: 1.5 }}>
              달러가 모인 체인이 곧 거래·디파이가 활발한 생태계 — <b style={{ color: TK.sub5 }}>자금이 어디로 이동하는지</b>(예: 솔라나 부상)를 읽는 단서.
            </div>
            {/* 누적 바 */}
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
              {d.chains.map((c, i) => (
                <div key={c.name} style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 11.5 }}>
                  ...
                  <span style={{ color: TK.slate200, fontWeight: 600, minWidth: 110 }}>{c.name}</span>
                  <span style={{ marginLeft: 'auto', color: TK.sub, fontFamily: 'monospace' }}>{fmtB(c.mcap)}</span>
                  <span style={{ color: TK.slate300, fontWeight: 700, fontFamily: 'monospace', minWidth: 44, textAlign: 'right' }}>{c.share}%</span>
                </div>
              ))}
            </div>
...
function DepegSimulator() {
  ...
  const Bar = ({ label, price, color, note }: { label: string; price: number; color: string; note: string }) => {
    ...
    return (
    <div style={{ flex: '1 1 200px', background: TK.bg3, border: `1px solid ${BORDER}`, borderRadius: 9, padding: '11px 13px' }}>
      <div style={{ color: TK.slate200, fontWeight: 700, fontSize: 12, marginBottom: 6 }}>{label}</div>
      <div style={{ position: 'relative', height: 12, background: TK.border, borderRadius: 6, marginBottom: 5 }}>
      ...
      <div style={{ color: price >= 0.97 ? TK.green400 : price >= 0.9 ? TK.amber400 : TK.red400, fontWeight: 900, fontSize: 18, fontFamily: 'monospace' }}>${price.toFixed(3)} <span style={{ fontSize: 11, color: TK.sub }}>(페그 −{lossPct}%)</span></div>
      {/* 💸 내 자산 손실 체감 */}
      <div style={{ marginTop: 5, fontSize: 11 }}>
        <span style={{ color: TK.sub }}>내 {fmtMan(amount * 1e4)}원 → </span>
  ...
  return (
    <div style={{ background: CARD, borderRadius: 12, border: `1px solid ${BORDER}`, padding: '14px 16px' }}>
      <div style={{ color: TK.slate200, fontWeight: 800, fontSize: 13, marginBottom: 3 }}>📉 디페깅 스트레스 시뮬레이터</div>
      <div style={{ color: TK.sub2, fontSize: 11, marginBottom: 11, lineHeight: 1.5 }}>시장 신뢰 충격(대량 매도)을 키워보세요 — 같은 충격에도 <b style={{ color: TK.green500 }}>법정담보</b>는 버티고 <b style={{ color: TK.red500 }}>알고리즘</b>은 어느 순간 붕괴합니다. <b style={{ color: TK.sub5 }}>내 투자금</b>을 넣으면 손실이 얼마인지 직접 보입니다.</div>

      <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 8 }}>
        <span style={{ color: TK.sub, fontSize: 11, minWidth: 90 }}>신뢰 충격(매도)</span>
        <input type="range" min={0} max={100} value={shock} onChange={e => setShock(Number(e.target.value))} style={{ flex: 1, accentColor: TK.red500 }} />
        <span style={{ color: TK.slate200, fontWeight: 800, fontSize: 14, fontFamily: 'monospace', minWidth: 44, textAlign: 'right' }}>{shock}%</span>
      </div>
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 14 }}>
        <span style={{ color: TK.sub, fontSize: 11, minWidth: 90 }}>내 투자금</span>
        <input value={amount} onChange={e => setAmount(Math.max(1, Number(e.target.value.replace(/[^0-9]/g, '')) || 1))}
          style={{ width: 70, background: TK.bg3, border: `1px solid ${BORDER}`, borderRadius: 6, padding: '4px 8px', color: TK.slate200, fontSize: 12.5, fontWeight: 800, fontFamily: 'monospace', textAlign: 'right' }} />
        <span style={{ color: TK.slate200, fontSize: 12, fontWeight: 700 }}>만원</span>
      </div>

      <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap' }}>
        <Bar label="🏦 법정화폐 담보 (USDC식)" price={sim.fiat} color={TK.green500} note="현금·국채를 1:1 보유 → 들고 오면 1달러로 상환. 일시 흔들려도 담보가 페그를 방어." />
        <Bar label="⚙️ 알고리즘 무담보 (UST식)" price={sim.algo} color={sim.spiral ? TK.red500 : TK.amber400} note={sim.spiral ? '🚨 데스 스파이럴! 페그 하락→자매코인 무한 발행→가격 폭락→더 큰 이탈. 2022 테라 $18B가 며칠 만에 $0.' : '담보 없이 차익거래로 페그 유지 — 신뢰가 깨지는 임계점(약 25%)까진 버티는 듯 보임.'} />
      </div>

      <div style={{ marginTop: 12, background: 'rgba(239,68,68,0.06)', border: '1px solid rgba(239,68,68,0.25)', borderRadius: 9, padding: '9px 12px', color: TK.sub15, fontSize: 11, lineHeight: 1.7 }}>
        🎓 <b style={{ color: TK.red400 }}>2022년 테라/루나 사태</b> — 알고리즘 스테이블코인 UST는 평소 $1을 잘 지켰지만, 대량 인출이 임계를 넘자 <b>며칠 만에 $1→$0.01</b>로 붕괴(시총 $18B 증발). &ldquo;평소 멀쩡하다&rdquo;가 안전을 보장하지 않습니다 — <b>담보의 질</b>이 본질입니다. 고수익(이자)을 미끼로 한 무담보 코인을 특히 경계하세요.
      </div>
    </div>
  )
}
