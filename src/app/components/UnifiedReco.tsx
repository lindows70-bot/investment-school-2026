  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
      ...
      <div style={{ color: TK.sub8, fontSize: 10.5, lineHeight: 1.6 }}>
        {/* ⚖️ 비중은 **데이터에서 그린다** — 여기가 리터럴로 "수급(연료) 포함 · 펀더멘탈 45%"라 굳어 있어
            같은 화면 상단("수급은 점수에서 빼고 참고 정보로만 · 펀더멘탈 50%")과 정면으로 모순이었다
            (2026-08-16 화면검증 — 종합판정 각주와 **같은 유형**의 리터럴 결함이 이 컴포넌트에도 있었다). */}
        ※ 통합 점수 = 💎가치(PEG 촘촘·어닝일드 E/P·FCF수익률) + 🏰퀄리티(영업이익률·ROE 자본효율·저부채 재무안정성·이익질) + 📈모멘텀(Fwd EPS·주가추세) + 🧭주도섹터(RRG 상대강도×모멘텀 — 지금 돈이 도는 섹터) + 🌦️계절(매크로 우대 섹터/분류). 펀더멘탈(가치+퀄리티)이 {pct(data.weights.value + data.weights.quality)}로 앵커입니다. 💰수급은 <b>점수에 안 들어갑니다</b>(아래 바는 참고용). 최종 선별 종목은 ⚙️ <b>ROIC 복리기계</b>(빚까지 반영한 정밀 자본효율)·📈 <b>Fwd EPS 리비전</b>으로 심화 검증해 배지로 표시합니다. <b>수급*</b>는 미국·유럽·일본·중국 종목으로, 해당 거래소가 외국인/기관 일별 수급을 공시하지 않아 MFI·내부자·13F 거인 <b>프록시</b>입니다(한국만 외인/기관/개인 실수급). 프록시조차 못 구한 종목은 <b>빗금(미집계)</b>으로 표시하고 중립 처리합니다 — 잰 척하지 않습니다. PEG는 stock-info SSOT 기준. 보유 종목은 제외했습니다. 교육용 시뮬레이션이며 투자 추천이 아닙니다.
      </div>
    </div>
  )
}
