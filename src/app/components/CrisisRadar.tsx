...
const SIG: Record<Signal, { dot: string; color: string; ko: string; bg: string }> = {
  safe: { dot: '🟢', color: TK.green400, ko: '안전', bg: 'rgba(74,222,128,0.08)' },
  caution: { dot: '🟡', color: TK.amber400, ko: '주의', bg: 'rgba(251,191,36,0.08)' },
  danger: { dot: '🔴', color: TK.red400, ko: '위험', bg: 'rgba(248,113,113,0.09)' },
}
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
      ...
      {d && (
        <div style={{ background: SIG[d.alertLevel].bg, border: `1px solid ${SIG[d.alertLevel].color}55`, borderRadius: 14, padding: '16px 20px' }}>
          ...
          <div style={{ color: TK.sub11, fontSize: 12.5, lineHeight: 1.7, marginTop: 8 }}>{d.summary}</div>
        </div>
      )}
      {!d && !err && <div style={{ background: CARD, borderRadius: 12, border: `1px solid ${BORDER}`, padding: 16, color: TK.sub, fontSize: 13 }}>버블 지표 4종을 실데이터로 계산 중입니다…</div>}
      ...
      {d && (
        <div style={{ background: CARD, borderRadius: 12, border: `1px solid ${BORDER}`, padding: '14px 16px' }}>
          <div style={{ color: TK.slate200, fontWeight: 800, fontSize: 13, marginBottom: 8 }}>🎛️ 한눈에 — 바늘이 빨강(위험)에 가까울수록 경고</div>
      ...
      {d && (
        <div style={{ background: CARD, borderRadius: 12, border: `1px solid ${BORDER}`, padding: '14px 16px' }}>
          <div style={{ color: TK.slate200, fontWeight: 800, fontSize: 13, marginBottom: 8 }}>📋 종합 평가 — 4대 지표 한 표로</div>
          <div style={{ overflowX: 'auto' }}>
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 11.5, minWidth: 560 }}>
              <thead>
                <tr style={{ color: TK.sub2, fontSize: 10, textAlign: 'left' }}>
                ...
                {d.metrics.map(m => {
                  ...
                  return (
                    <tr key={m.key} style={{ borderTop: `1px solid ${BORDER}` }}>
                      <td style={{ padding: '8px', color: TK.slate200, fontWeight: 700 }}>{m.icon} {m.label}</td>
                      <td style={{ padding: '8px', color: TK.sub8, fontSize: 10.5 }}>{m.measure}</td>
                      <td style={{ padding: '8px', textAlign: 'right', color: TK.sub8, fontFamily: 'monospace' }}>{m.mean}{m.unit}</td>
          ...
          <div style={{ color: TK.sub2, fontSize: 9.5, marginTop: 6 }}>💡 선행 PER(뉴스 ~20배)은 미래 예상이익 기준 &lsquo;주의&rsquo;, 우리 후행 PER(32배)은 확정이익 기준 &lsquo;위험&rsquo; — 둘 다 맞고, 우리가 더 보수적(선행은 유료 데이터).</div>
        </div>
      )}
      ...
      {d && (
        <div style={{ color: TK.sub2, fontSize: 10, lineHeight: 1.6, padding: '0 4px' }}>
          ⚠️ 데이터: Shiller CAPE·S&P PER·어닝일드(multpl.com)·버핏지표(FRED 시총÷GDP, 과거값도 실측)·국채금리(FRED). 임계 밴드는 공개 방법론의 교과서 기준(교육용). <b style={{ color: TK.sub11 }}>밸류에이션 지표는 &lsquo;언제&rsquo; 떨어질지는 못 맞힙니다</b> — 고평가에서 몇 년 더 오르기도 합니다. 폭락 예언이 아니라 위험 관리·기대수익 조정 신호로 쓰세요. 레이 달리오 버블 지표·매크로 날씨와 함께 보면 좋습니다.
        </div>
      )}
    </div>
  )
...
function MetricCard({ m }: { m: CrisisMetric }) {
  ...
  const crises = m.key === 'erp' ? [] : [['2000-03', '2000', TK.red400], ['2007-09', '2008', TK.orange400], ['2020-03', '2020', TK.purple500]] as const
  return (
    <div style={{ background: CARD, borderRadius: 12, border: `1px solid ${BORDER}`, padding: '14px 16px' }}>
      <div style={{ display: 'flex', alignItems: 'baseline', gap: 10, flexWrap: 'wrap' }}>
        <span style={{ fontSize: 17 }}>{m.icon}</span>
        <span style={{ color: TK.slate200, fontWeight: 800, fontSize: 14 }}>{m.label}</span>
        <span style={{ color: TK.sub, fontSize: 11 }}>{m.measure}</span>
      ...
      {/* 역사 미니차트 */}
      {m.key === 'pe' && <div style={{ color: TK.sub2, fontSize: 9.5, marginTop: 6 }}>📉 아래 차트는 <b style={{ color: TK.sub8 }}>후행 PER 30년 추이</b>(선행 PER 장기 시계열은 무료 미제공) — 2008년 스파이크는 이익 급감으로 PER이 역설적으로 치솟은 것</div>}
      {m.series.length > 8 && (
        <div style={{ height: 150, marginTop: 8 }}>
          <ResponsiveContainer width="100%" height="100%">
            <AreaChart data={m.series} margin={{ top: 8, right: 10, left: -8, bottom: 0 }}>
              ...
              <XAxis dataKey="date" tick={{ fill: TK.sub2, fontSize: 8.5 }} minTickGap={55} axisLine={{ stroke: BORDER }} tickLine={false} tickFormatter={(v: string) => v.slice(0, 4)} />
              <YAxis tick={{ fill: TK.sub2, fontSize: 8.5 }} axisLine={false} tickLine={false} width={34} tickFormatter={(v: number) => `${v}${m.unit === '%' ? '%' : ''}`} />
              {/* eslint-disable-next-line @typescript-eslint/no-explicit-any */}
              <Tooltip contentStyle={{ background: TK.bg3, border: `1px solid ${BORDER}`, borderRadius: 8, fontSize: 11 }} formatter={(v: any) => [`${v}${m.unit}`, m.label]} />
              <ReferenceLine y={meanY} stroke={TK.green400} strokeDasharray="4 4" label={{ value: `평균 ${meanY}`, fill: TK.green400, fontSize: 8.5, position: 'insideBottomLeft' }} />
              <ReferenceLine y={dangerY} stroke={TK.red400} strokeDasharray="4 4" label={{ value: `위험 ${dangerY}`, fill: TK.red400, fontSize: 8.5, position: 'insideTopLeft' }} />
      ...
      {/* 쉬운 설명 */}
      <div style={{ background: TK.bg3, borderRadius: 9, borderLeft: `3px solid ${s.color}`, padding: '9px 12px', marginTop: 8 }}>
        <div style={{ color: s.color, fontWeight: 800, fontSize: 11.5, marginBottom: 3 }}>🎓 쉽게 말하면</div>
        <div style={{ color: TK.sub11, fontSize: 11.5, lineHeight: 1.7 }} dangerouslySetInnerHTML={{ __html: mdBold(m.explain) }} />
      </div>

      {/* 현재 신호 해석 + 과거위기 칩 */}
      <div style={{ color: TK.sub8, fontSize: 11, lineHeight: 1.6, marginTop: 7 }}>{m.note}</div>
      {m.history && (
        <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginTop: 7 }}>
          {m.history.map(h => (
            <div key={h.label} style={{ background: TK.bg6, borderRadius: 7, padding: '4px 10px', border: `1px solid ${BORDER}` }}>
              <span style={{ color: TK.sub2, fontSize: 9 }}>{h.label} </span>
              <span style={{ color: TK.sub11, fontWeight: 800, fontSize: 12, fontFamily: 'monospace' }}>{h.value}{m.unit}</span>
            </div>
          ))}
          {m.value != null && (
...
function mdBold(t: string): string {
  return t.replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/\*\*(.+?)\*\*/g, `<b style="color:${TK.slate200}">$1</b>`)
}
...
function Gauge({ m }: { m: CrisisMetric }) {
  ...
  const segs = invert
    ? [[min, t2, TK.red400], [t2, t1, TK.amber400], [t1, max, TK.green400]]
    : [[min, t1, TK.green400], [t1, t2, TK.amber400], [t2, max, TK.red400]]
  const nv = m.value ?? min
  return (
    <div style={{ background: TK.bg3, borderRadius: 9, border: `1px solid ${BORDER}`, padding: '8px 8px 6px', textAlign: 'center' }}>
      <svg viewBox="0 0 160 90" style={{ width: '100%', height: 'auto', display: 'block' }}>
        ...
        <text x={cx} y={cyy - 7} fill={TK.sub2} fontSize="8" textAnchor="middle">{m.unit}</text>
      </svg>
      <div style={{ color: TK.slate200, fontWeight: 700, fontSize: 11, marginTop: 2 }}>{m.icon} {m.label}</div>
