...
export default function FcfQualityCard({ ticker, name, market }: { ticker: string; name: string; market: string }) {
  ...
  const badge = (() => {
    switch (d.grade) {
      ...
      case 'expensive': return { t: `💵 FCF 수익률 ${d.fcfYield}% · 현금 대비 고평가`, c: TK.orange400, bg: '#7c2d1233', sub: '현금창출 대비 주가가 비쌈 — 버블·하락장에서 취약할 수 있음(멀티플 의존)' }
      case 'capex': return { t: '💵 FCF 적자(영업현금은 흑자)', c: TK.cyan400, bg: '#0e494933', sub: '영업현금(OCF)은 흑자인데 CAPEX 성장 투자로 FCF 적자 — 좀비 아님(투자 회수 여부가 관건)' }
      case 'loss': return { t: '💵 FCF·영업현금 적자', c: TK.red400, bg: '#7f1d1d33', sub: '현금 창출력 약화 — 현금 런웨이·흑자전환 먼저 확인' }
      default: return null
    }
  })()
  ...
  return (
    <div style={{ background: C.panel, border: `1px solid ${C.grid}`, borderRadius: 12, padding: 12 }}>
      <div style={{ fontWeight: 800, color: TK.violet300, fontSize: 12, marginBottom: 8 }}>
      ...
      {d.isFinancial ? (
        <div style={{ fontSize: 11, color: C.low, lineHeight: 1.6 }}>
          🏦 <b style={{ color: TK.slate400 }}>금융주(은행·보험·증권)</b> — 영업현금흐름·FCF가 예금·대출·트레이딩·보험 float으로 출렁여 &lsquo;이익의 질&rsquo; 신호가 무의미합니다. FCF 지표는 중립 처리하고 <b>P/B·ROE·내재가치(EV)</b>로 평가하세요.
        </div>
      ) : badge ? (<>
        ...
        <div style={{ marginTop: 8, display: 'grid', gridTemplateColumns: 'auto 1fr', columnGap: 12, rowGap: 3, fontSize: 10.5 }}>
          ...
          <span style={{ color: d.ocf != null && d.ocf < 0 ? TK.red400 : TK.green400, fontFamily: 'monospace' }}>{d.ocf != null ? (d.ocf >= 0 ? '흑자' : '적자') : '—'}{d.fcf != null ? ` · FCF ${d.fcf >= 0 ? '흑자' : '적자'}` : ''}</span>
        </div>
      </>) : null}

      <div style={{ marginTop: 8, fontSize: 9.5, color: TK.sub2, borderTop: `1px solid ${C.grid}`, paddingTop: 6, lineHeight: 1.5 }}>
