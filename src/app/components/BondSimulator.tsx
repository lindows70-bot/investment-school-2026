...
// ── 퀴즈 데이터 ───────────────────────────────────────────────────────────
// ⚠️ 2026-08-30 감사 — 예전엔 -14.7%/+19.9% 가 손으로 박혀 있었는데, **같은 화면의 bondPV 모델**로
//    재계산하면 -17.3%/+22.4% 라 학생이 슬라이더로 바로 반증할 수 있었다(⛔ 출처 없는 수치 금지).
//    이제 퀴즈 수치는 시뮬레이터와 **같은 함수**에서 파생된다 — 두 표면이 어긋날 수 없다.
const Q30_BASE = bondPV(0.03, 30)
const Q30_UP   = +((bondPV(0.04, 30) / Q30_BASE - 1) * 100).toFixed(1)   // 금리 +1%p
const Q30_DOWN = +((bondPV(0.02, 30) / Q30_BASE - 1) * 100).toFixed(1)   // 금리 −1%p

const QUIZZES = [
  {
    q: 'Q1. 금리가 1% 상승하면 30년 초장기채 가격은 어떻게 될까요?',
    options: [`A) 약 +${Math.abs(Q30_UP)}% 상승`, `B) 약 ${Q30_UP}% 하락`, 'C) 변화 없음', 'D) 약 -1% 하락'],
    answer: 1,
    explanation:
      `✅ 정답: B) 약 ${Q30_UP}% 하락 (표면금리 3%·기준금리 3% 가정 — 이 화면의 계산식과 동일)\n\n듀레이션이 길수록 금리 변화에 대한 가격 민감도가 높습니다. 30년 초장기채는 수십 번의 이자 흐름이 모두 금리 변화에 영향받아 단기채보다 훨씬 크게 하락합니다. 이것이 "장기채 투자 리스크"입니다.`,
  },
  {
    ...
    answer: 1,
    explanation:
      `✅ 정답: B) 비대칭성\n\n볼록성의 핵심은 비대칭입니다. 30년 초장기채 기준 금리 1%p 상승 시 ${Q30_UP}% 손실이지만, 금리 1%p 하락 시 +${Q30_DOWN}% 수익입니다(이 화면의 계산식 기준). 투자자 입장에서는 "손실보다 이익이 더 크다"는 특성이 있습니다. 금리 인하 사이클에서 장기채가 크게 수익을 내는 이유입니다.`,
  },
  {
