...
const SUB = { foreign: { label: '외국인', color: '#f59e0b' }, organ: { label: '기관', color: '#22d3ee' }, individual: { label: '개인', color: '#94a3b8' } } as const

function Cell({ amt, maxAbs }: { amt: number; maxAbs: number }) {
  const buy = amt > 0
  const col = Math.abs(amt) < 0.05 ? '#64748b' : buy ? '#22c55e' : '#ef4444'
...
export default function InvestorTimeline({ ticker, name }: { ticker: string; name: string }) {
  ...
  if (loading) return <div style={{ padding: '10px 14px', color: '#8a9aaa', fontSize: 12 }}>📅 일별 매매동향 불러오는 중…</div>
  if (err || !data) return <div style={{ padding: '10px 14px', color: '#8a9aaa', fontSize: 12 }}>일별 매매동향을 불러오지 못했습니다(국내 개별주식만).</div>
  ...
  return (
    <div style={{ padding: '10px 12px 4px', background: '#0b0e15', borderRadius: 8 }}>
      ...
      <div style={{ display: 'flex', gap: 8, marginBottom: 10, flexWrap: 'wrap' }}>
        <span style={{ color: '#7f93a8', fontSize: 11, alignSelf: 'center' }}>최근 {data.days}일 누적</span>
        {(['foreign', 'organ', 'individual'] as const).map(k => {
          const v = data.cum[k], cfg = SUB[k], buy = v > 0
          return (
            <span key={k} style={{ display: 'inline-flex', alignItems: 'center', gap: 5, background: '#0f1117', border: `1px solid ${cfg.color}44`, borderRadius: 8, padding: '4px 10px', fontSize: 11.5 }}>
              <span style={{ color: cfg.color, fontWeight: 700 }}>{cfg.label}</span>
              <span style={{ color: buy ? '#22c55e' : v < 0 ? '#ef4444' : '#8a9aaa', fontWeight: 800, fontFamily: 'monospace' }}>{fmtEok(v)}</span>
            </span>
          )
        })}
      </div>
      ...
      {/* 테이블 헤더 */}
      <div style={{ display: 'grid', gridTemplateColumns: '58px 1fr 84px 84px 84px', gap: 8, padding: '0 2px 5px', fontSize: 10, color: '#7f93a8', borderBottom: '1px solid #1e293b' }}>
          ...
          return (
            <div key={r.date} style={{ display: 'grid', gridTemplateColumns: '58px 1fr 84px 84px 84px', gap: 8, padding: '6px 2px', alignItems: 'center', borderBottom: '1px solid #131922' }}>
              ...
              <span style={{ fontSize: 11.5 }}>
                <span style={{ color: '#cbd5e1', fontFamily: 'monospace' }}>{r.close.toLocaleString()}</span>
                <span style={{ color: r.changePct == null ? '#64748b' : up ? '#22c55e' : '#ef4444', marginLeft: 5, fontSize: 10.5 }}>
