...
const Kpi = ({ label, value, sub, color = TK.slate200 }: { label: string; value: string; sub?: string; color?: string }) => (
  <div style={{ background: CARD, border: `1px solid ${BORDER}`, borderRadius: 12, padding: '12px 16px', minWidth: 150, flex: '1 1 150px' }}>
    <div style={{ color: TK.sub, fontSize: 11, fontWeight: 700 }}>{label}</div>
    <div style={{ color, fontSize: 20, fontWeight: 900, marginTop: 3, fontFamily: 'monospace' }}>{value}</div>
    {sub && <div style={{ color: TK.sub, fontSize: 10.5, marginTop: 2 }}>{sub}</div>}
  </div>
)
...
const Panel = ({ title, sub, children }: { title: string; sub?: string; children: React.ReactNode }) => (
  <div style={{ background: CARD, border: `1px solid ${BORDER}`, borderRadius: 14, padding: '16px 18px' }}>
    ...
    {sub && <div style={{ color: TK.sub, fontSize: 11, margin: '3px 0 10px', lineHeight: 1.55 }}>{sub}</div>}
    {children}
  </div>
)

const TT = { contentStyle: { background: TK.bg3, border: `1px solid ${BORDER}`, borderRadius: 8, fontSize: 11 }, labelStyle: { color: TK.slate300 } }
const AXIS = { fill: TK.sub, fontSize: 10 }
// 20년 월별 X축 — 연 단위 눈금만
...
export default function ReMarketDashboard() {
  ...
  if (err) return <div style={{ background: CARD, borderRadius: 12, padding: 24, border: `1px solid ${BORDER}`, color: TK.red400, fontSize: 12 }}>⚠️ 부동산 시장 데이터를 불러오지 못했습니다 — 잠시 후 새로고침해주세요.</div>
  if (!d) return <div style={{ background: CARD, borderRadius: 12, padding: 24, border: `1px solid ${BORDER}`, color: TK.sub, fontSize: 12 }}>🏠 부동산 시장 데이터를 모으는 중… (한국은행 ECOS·FRED)</div>

  const k = d.kpi
  const unsoldTone = k.unsoldPercentile == null ? TK.slate200 : k.unsoldPercentile >= 75 ? TK.red400 : k.unsoldPercentile >= 50 ? TK.amber400 : TK.green400
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 10 }}>
        ...
        <Kpi label="KB 아파트 매매지수 YoY" value={k.kbAptYoY != null ? `${k.kbAptYoY > 0 ? '+' : ''}${k.kbAptYoY}%` : '—'}
          sub={`전국 · ${k.asOfKb ?? ''}`} color={k.kbAptYoY == null ? TK.slate200 : k.kbAptYoY > 0 ? TK.red400 : TK.blue400} />
      ...
      <Panel title="📉 KB 아파트 매매·전세 지수 × 기준금리 — 금리는 집값의 중력"
        sub="금리가 오르면(보라, 우축) 대출 비용이 늘어 집값 지수(좌축)가 눌리고, 금리가 내리면 다시 뜨는 역관계를 20년 시계열로 확인합니다. 전세(초록)는 '사용가치' — 매매와의 간격이 벌어지면 거품/저평가 신호의 단서. ⓘ 지수는 2026-01=100 재기준이라 선의 '높이'가 아닌 기울기(변화율)로 읽으세요 — 과거 구간에서 서울(주황)이 전국보다 낮게 보이는 건 서울이 그만큼 이후 더 올랐다는 뜻(가격이 쌌다는 게 아님).">
        <ResponsiveContainer width="100%" height={300}>
          <ComposedChart data={d.kbChart} margin={{ top: 6, right: 8, left: 0, bottom: 0 }}>
            ...
            <Line yAxisId="idx" type="monotone" dataKey="sale" name="아파트 매매(전국)" stroke={TK.slate100} strokeWidth={2} dot={false} connectNulls />
            <Line yAxisId="idx" type="monotone" dataKey="saleSeoul" name="아파트 매매(서울)" stroke={TK.orange400} strokeWidth={1.4} dot={false} connectNulls />
            <Line yAxisId="idx" type="monotone" dataKey="jeonse" name="아파트 전세(전국)" stroke={TK.green400} strokeWidth={1.4} dot={false} connectNulls />
            <Line yAxisId="rate" type="stepAfter" dataKey="baseRate" name="기준금리(우)" stroke={TK.violet400} strokeWidth={1.6} dot={false} connectNulls />
          </ComposedChart>
        </ResponsiveContainer>
      </Panel>
      ...
      <Panel title="🏷️ 아파트 매매 실거래가격지수 — 호가가 아닌 '실제 체결가'의 흐름"
        ...
        <ResponsiveContainer width="100%" height={260}>
          <ComposedChart data={d.rtChart} margin={{ top: 6, right: 8, left: 0, bottom: 0 }}>
            ...
            <Tooltip {...TT} />
            <Legend wrapperStyle={{ fontSize: 11 }} />
            <Line type="monotone" dataKey="nation" name="전국" stroke={TK.slate100} strokeWidth={1.8} dot={false} connectNulls />
            <Line type="monotone" dataKey="seoul" name="서울" stroke={TK.orange400} strokeWidth={1.6} dot={false} connectNulls />
            <Line type="monotone" dataKey="capital" name="수도권" stroke={TK.blue400} strokeWidth={1.3} dot={false} connectNulls />
          </ComposedChart>
        </ResponsiveContainer>
      </Panel>
      ...
      <Panel title="📦 미분양 주택 — 부동산판 '재고 적체' 레이더"
        ...
        <ResponsiveContainer width="100%" height={240}>
          <ComposedChart data={d.unsoldChart} margin={{ top: 6, right: 8, left: 0, bottom: 0 }}>
            ...
            <Legend wrapperStyle={{ fontSize: 11 }} />
            <Area type="monotone" dataKey="nation" name="전국" stroke={TK.orange400} fill={`${TK.orange400}22`} strokeWidth={1.8} connectNulls />
            <Line type="monotone" dataKey="capital" name="수도권" stroke={TK.blue400} strokeWidth={1.4} dot={false} connectNulls />
          </ComposedChart>
        </ResponsiveContainer>
      </Panel>
      ...
      <Panel title="🇺🇸 미국 집값(케이스-실러) × 모기지 30년 금리 — 글로벌 비교축"
        ...
        <ResponsiveContainer width="100%" height={240}>
          <ComposedChart data={d.usChart} margin={{ top: 6, right: 8, left: 0, bottom: 0 }}>
            ...
            <Tooltip {...TT} />
            <Legend wrapperStyle={{ fontSize: 11 }} />
            <Line yAxisId="cs" type="monotone" dataKey="caseShiller" name="케이스-실러 집값지수" stroke={TK.slate100} strokeWidth={1.8} dot={false} connectNulls />
            <Line yAxisId="mr" type="monotone" dataKey="mortgage" name="모기지 30년(우)" stroke={TK.violet400} strokeWidth={1.4} dot={false} connectNulls />
          </ComposedChart>
        </ResponsiveContainer>
      </Panel>
      ...
      <div style={{ background: CARD, border: `1px solid ${BORDER}`, borderRadius: 14, padding: '14px 18px', fontSize: 11, color: TK.sub5, lineHeight: 1.7 }}>
        <b style={{ color: TK.slate200 }}>🧭 부동산 4축 프레임(주식과 같은 언어)</b> — 이 화면은 1단계(시장 전체). 다음 단계로
