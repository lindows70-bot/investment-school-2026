...
function getPolicyStance(data: InflationPoint[]): {
  ...
  if (!latest) return { label: '로딩 중', desc: '—', color: TK.slate400, bg: 'rgba(148,163,184,0.08)', border: 'rgba(148,163,184,0.2)' }
  const spread = latest.fedRate - latest.corePCE
  if (spread > 1.0) return {
    label: 'RESTRICTIVE', desc: '제약적 통화정책',
    color: TK.red400, bg: 'rgba(248,113,113,0.08)', border: 'rgba(248,113,113,0.25)',
  }
  if (spread > 0) return {
    label: 'MILDLY TIGHT', desc: '완만한 긴축',
    color: TK.orange400, bg: 'rgba(251,146,60,0.08)', border: 'rgba(251,146,60,0.25)',
  }
  return {
    label: 'NEUTRAL / EASY', desc: '중립 또는 완화',
    color: TK.green400, bg: 'rgba(74,222,128,0.08)', border: 'rgba(74,222,128,0.25)',
  }
}
...
function buildSynthesis(
  ...
  if (net >= 2)
    return { label: '종합: 공격 우호', body: '실질금리 완화·유동성·금리 인하가 위험자산에 우호적 — 우량 성장주 중심 적극 운용이 유리합니다.', color: TK.green400, bg: 'rgba(74,222,128,0.08)', border: 'rgba(74,222,128,0.3)' }
  if (net <= -3)
    return { label: '종합: 방어 우선', body: '제약적 금리·유동성 흡수·금리 동결/인상이 겹치는 방어적 환경 — 현금·단기채·이자수익 우량주 비중을 높이세요.', color: TK.red400, bg: 'rgba(248,113,113,0.08)', border: 'rgba(248,113,113,0.3)' }
  return { label: '종합: 중립~방어 — 우량주 선별', body: '실질금리 제약은 완화됐으나 유동성(QT)·금리 방향(동결/인상)이 아직 보수적입니다. 적극 추격보다 우량주를 선별 매수하는 구간입니다.', color: TK.amber400, bg: 'rgba(251,191,36,0.08)', border: 'rgba(251,191,36,0.3)' }
}
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          ...
          <div>
            <div style={{ fontSize: 15, fontWeight: 900, color: TK.slate100 }}>Fed Watch — 거시경제 모니터</div>
            <div style={{ fontSize: 11, color: TK.sub3, marginTop: 2 }}>연준 통화정책 · 인플레이션 · 유동성 지표 실시간 추적</div>
          </div>
        </div>
        ...
        <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', alignItems: 'center' }}>
          ...
          <div style={{
            ...
            <span style={{ fontSize: 10, color: TK.sub3 }}>{policyStance.desc}</span>
          </div>
          ...
          <div style={{
            ...
            <span style={{ fontSize: 10, color: TK.sub3 }}>기준금리</span>
            <span style={{ fontSize: 13, fontWeight: 900, color: TK.blue400, fontFamily: 'monospace' }}>
          ...
          <div style={{
            ...
            <span style={{ fontSize: 10, color: TK.sub3 }}>Core PCE</span>
          ...
          <div style={{
            ...
            <span style={{ fontSize: 10, color: TK.sub3 }}>다음 FOMC</span>
            <span style={{ fontSize: 11, fontWeight: 700, color: TK.amber400 }}>{nextFomc.date}</span>
            <span style={{ fontSize: 9, padding: '1px 6px', borderRadius: 4, background: 'rgba(251,191,36,0.15)', color: TK.amber400, fontWeight: 800 }}>
              D-{nextFomc.dDay}
            </span>
          </div>
          ...
          <div style={{
            ...
            fontSize: 10, color: inflMock ? TK.amber400 : TK.emerald400,
          }}>
      ...
      <div style={{
        ...
        <div style={{ fontSize: 12, fontWeight: 800, color: TK.slate100, marginBottom: 10 }}>
          💡 현재 매크로 환경 기반 포지션 가이드
          {(!inflLoading && !bsLoading && !inflMock && !bsMock) && (
            <span style={{ marginLeft: 8, fontSize: 10, color: TK.emerald400, fontWeight: 600 }}>
              🟢 실시간 FRED 데이터 반영
            </span>
          )}
          {(inflMock || bsMock) && (
        ...
        {(inflLoading || bsLoading) && (
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: 10 }}>
            {[0, 1, 2].map(i => (
              <div key={i} style={{ height: 80, background: TK.bg10, borderRadius: 10, animation: 'pulse 1.5s infinite' }} />
            ))}
          </div>
        )}
        ...
        {synthesis && (
          <div style={{ display: 'flex', alignItems: 'flex-start', gap: 8, padding: '10px 14px', borderRadius: 10, background: synthesis.bg, border: `1px solid ${synthesis.border}`, marginBottom: 10 }}>
            ...
            <span style={{ fontSize: 11.5, color: TK.sub5, lineHeight: 1.5 }}>{synthesis.body}</span>
          </div>
        )}
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: 10 }}>
            {guideCards.map((card, i) => (
              <div key={i} style={{
                ...
                <div style={{ fontSize: 11, color: TK.sub2, lineHeight: 1.65 }}>{card.body}</div>
              </div>
            ))}
          </div>
        )}
