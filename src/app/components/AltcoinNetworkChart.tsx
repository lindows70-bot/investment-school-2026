...
const DIV_META: Record<AltCoin['divergence'], { c: string; label: string }> = {
  hype:    { c: TK.red500, label: '투기 프리미엄 의심' },
  healthy: { c: TK.green500, label: '펀더멘탈 동반 성장' },
  value:   { c: TK.amber400, label: '디커플링(저평가 여지)' },
  neutral: { c: TK.slate400, label: '중립' },
}
...
function CoinChart({ c }: { c: AltCoin }) {
  const col = SYM_COLOR[c.symbol] ?? TK.violet400
...
export default function AltcoinNetworkChart() {
  ...
  if (loading) return <div style={{ background: CARD, borderRadius: 12, padding: 24, border: `1px solid ${BORDER}`, color: TK.sub, fontSize: 12 }}>🔷 알트코인 네트워크 데이터(가격·TVL·거래량)를 모으는 중…</div>
  if (!d || d.coins.length === 0) return <div style={{ background: CARD, borderRadius: 12, padding: 24, border: `1px solid ${BORDER}`, color: TK.sub, fontSize: 12 }}>알트코인 데이터를 불러오지 못했습니다 — 잠시 후 새로고침해주세요.</div>
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
      <div style={{ background: 'linear-gradient(135deg,rgba(98,126,234,0.12),rgba(20,241,149,0.05))', border: '1px solid rgba(98,126,234,0.35)', borderRadius: 12, padding: '12px 16px' }}>
        <span style={{ color: TK.indigo300, fontWeight: 800, fontSize: 14 }}>🔷 메이저 알트코인 — 가격 vs 네트워크 펀더멘탈</span>
        <div style={{ color: TK.sub5, fontSize: 11.5, lineHeight: 1.6, marginTop: 4 }}>
