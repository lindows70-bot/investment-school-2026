...
const TICK = { fill: TK.sub2, fontSize: 9.5 }
const TIP_STYLE = { background: TK.bg3, border: `1px solid ${BORDER}`, borderRadius: 8, fontSize: 11 }
  ...
  return (
    <div style={{ flex: '1 1 420px', minWidth: 0, background: CARD, borderRadius: 12, padding: '13px 15px', border: `1px solid ${BORDER}` }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap', marginBottom: 2 }}>
        <span style={{ color: TK.slate200, fontWeight: 800, fontSize: 12.5 }}>{title}</span>
        <span style={{ color: TK.sub, fontSize: 10.5 }}>{sub}</span>
      ...
      <div style={{ color: TK.sub8, fontSize: 10.5, lineHeight: 1.55, marginTop: 6 }}>{note}</div>
    </div>
  )
}
...
export default function FedChartsBoard() {
  ...
  if (!d) return <div style={{ background: CARD, borderRadius: 12, padding: 24, border: `1px solid ${BORDER}`, color: TK.sub, fontSize: 12 }}>📊 연준 핵심지표 차트를 불러오는 중…</div>
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
      <div style={{ background: 'linear-gradient(135deg,rgba(96,165,250,0.10),rgba(34,197,94,0.05))', border: '1px solid rgba(96,165,250,0.3)', borderRadius: 12, padding: '11px 16px' }}>
        <span style={{ color: TK.slate200, fontWeight: 800, fontSize: 14 }}>📊 연준 핵심지표 차트보드</span>
        <span style={{ color: TK.sub, fontSize: 11, marginLeft: 10 }}>물가의 상류→하류, 고용, 연준의 실탄까지 — 추세로 읽는 양대책무</span>
      </div>
      ...
      <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap' }}>
        ...
        <Panel title="① 물가 3겹 — 노이즈 벗기기" sub="전년비 % · 5년" note={d.inflationNote}>
          <ResponsiveContainer width="100%" height="100%">
            <LineChart data={d.inflation} margin={{ top: 6, right: 8, left: -18, bottom: 0 }}>
              ...
              <Tooltip contentStyle={TIP_STYLE} labelStyle={{ color: TK.sub }} formatter={v => `${v}%`} />
              <Legend wrapperStyle={{ fontSize: 10 }} />
              <ReferenceLine y={2} stroke={TK.slate400} strokeDasharray="4 3" label={{ value: '목표 2%', fill: TK.slate400, fontSize: 9, position: 'insideRight', dy: -9 }} />
              <Line name="헤드라인 CPI" dataKey="headline" stroke={TK.orange400} dot={false} strokeWidth={1.8} connectNulls />
              <Line name="근원 CPI" dataKey="core" stroke={TK.blue400} dot={false} strokeWidth={1.6} strokeDasharray="5 3" connectNulls />
              <Line name="근원 PCE (연준 공식 목표)" dataKey="corePce" stroke={TK.amber400} dot={false} strokeWidth={1.8} connectNulls />
              <Line name="절사평균 PCE (워시 픽)" dataKey="trimmed" stroke={TK.cyan400} dot={false} strokeWidth={2.2} connectNulls />
            </LineChart>
          </ResponsiveContainer>
        </Panel>
        ...
        <Panel title="② PPI — 물가의 상류" sub="최종수요 전월비 % · 36개월" note={d.ppiNote}
          badge={d.ppiReaccel ? { text: '🚨 CPI 재가속 경고', color: TK.red400 } : { text: '추세 범위', color: TK.green400 }}>
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={d.ppi} margin={{ top: 6, right: 8, left: -18, bottom: 0 }}>
              <XAxis dataKey="date" tick={TICK} tickFormatter={fmtYm} minTickGap={42} axisLine={{ stroke: BORDER }} tickLine={false} />
              <YAxis tick={TICK} axisLine={false} tickLine={false} />
              <Tooltip contentStyle={TIP_STYLE} labelStyle={{ color: TK.sub }} formatter={v => `${typeof v === 'number' && v > 0 ? '+' : ''}${v}%`} />
              <ReferenceLine y={0} stroke={BORDER} />
              <Bar dataKey="v" name="PPI MoM" radius={[2, 2, 0, 0]}>
                {d.ppi.map(o => <Cell key={o.date} fill={o.v >= 0 ? TK.red400 : TK.blue400} />)}
              </Bar>
            </BarChart>
          </ResponsiveContainer>
        </Panel>
        ...
        <Panel title="③ 고용 듀얼" sub="비농업 MoM(천명) + 실업률 % · 24개월" note={d.laborNote}>
          <ResponsiveContainer width="100%" height="100%">
            <ComposedChart data={d.labor} margin={{ top: 6, right: -8, left: -18, bottom: 0 }}>
              ...
              <Tooltip contentStyle={TIP_STYLE} labelStyle={{ color: TK.sub }} />
              <Legend wrapperStyle={{ fontSize: 10 }} />
              <ReferenceLine yAxisId="l" y={0} stroke={BORDER} />
              <Bar yAxisId="l" dataKey="payemsK" name="비농업 MoM(K)" fill={TK.green400} radius={[2, 2, 0, 0]}>
                {d.labor.map(o => <Cell key={o.date} fill={o.payemsK >= 0 ? TK.green400 : TK.red400} />)}
              </Bar>
              <Line yAxisId="r" dataKey="unrate" name="실업률 %" stroke={TK.amber400} dot={false} strokeWidth={1.8} connectNulls />
            </ComposedChart>
          </ResponsiveContainer>
        </Panel>
        ...
        <Panel title="④ 연준의 실탄" sub="FF금리(월말) vs 절사평균 PCE · 5년" note={d.ammoNote}>
          <ResponsiveContainer width="100%" height="100%">
            <LineChart data={d.ammo} margin={{ top: 6, right: 8, left: -18, bottom: 0 }}>
              ...
              <Tooltip contentStyle={TIP_STYLE} labelStyle={{ color: TK.sub }} formatter={v => `${v}%`} />
              <Legend wrapperStyle={{ fontSize: 10 }} />
              <Line name="기준금리(상단)" dataKey="ffr" stroke={TK.red400} dot={false} strokeWidth={2} connectNulls />
              <Line name="절사평균 PCE" dataKey="trimmed" stroke={TK.cyan400} dot={false} strokeWidth={2} connectNulls />
              <Line name="갭(실탄 %p)" dataKey="gap" stroke={TK.violet400} dot={false} strokeWidth={1.4} strokeDasharray="4 3" connectNulls />
            </LineChart>
          </ResponsiveContainer>
        </Panel>
      </div>
      ...
      <div style={{ background: CARD, borderRadius: 12, border: `1px solid ${BORDER}` }}>
        <button onClick={() => setEduOpen(o => !o)} style={{ width: '100%', display: 'flex', alignItems: 'center', gap: 8, background: 'transparent', border: 'none', cursor: 'pointer', padding: '11px 15px', textAlign: 'left' }}>
          <span style={{ color: TK.amber400, fontWeight: 800, fontSize: 12.5 }}>🎓 왜 연준은 CPI가 아니라 PCE를 보나요?</span>
          <span style={{ color: TK.sub, fontSize: 10.5 }}>뉴스는 CPI를 외치지만, 금리를 정하는 건 PCE입니다</span>
        ...
        {eduOpen && (
          <div style={{ padding: '0 15px 13px', display: 'flex', flexDirection: 'column', gap: 8 }}>
            ...
            <div style={{ background: 'rgba(245,158,11,0.07)', border: '1px solid rgba(245,158,11,0.3)', borderRadius: 9, padding: '9px 12px', color: TK.sub15, fontSize: 11, lineHeight: 1.7 }}>
              🎓 <b style={{ color: TK.amber400 }}>최일 쌤의 실전 팁</b> — 그래서 연준은 날씨·유가 따라 널뛰는 헤드라인보다 노이즈를 걷어낸 <b>근원 PCE</b>(노란 선)와, 워시 의장이 강조하는 <b>달라스 연준 절사평균 PCE</b>(하늘색 선)를 나침반으로 삼습니다. 위 ① 차트에서 네 선의 간격 자체가 &lsquo;노이즈의 두께&rsquo;입니다 — 뉴스가 CPI 공포를 외칠 때, 연준의 눈이 보는 선이 어디 있는지 먼저 확인하세요.
            </div>
          </div>
        )}
      </div>
