...
const V: Record<DcfVerdict, { label: string; color: string; icon: string }> = {
  demanding:    { label: '기대 과도', color: TK.red500, icon: '🔥' },
  fair:         { label: '합리적',    color: TK.amber400, icon: '⚖️' },
  conservative: { label: '기대 보수 (저평가 여지)', color: TK.green500, icon: '🌱' },
  unknown:      { label: '판단 보류', color: TK.sub, icon: '❔' },
}
...
export default function ReverseDcf({ ticker, name, market }: { ticker: string; name: string; market: string }) {
  ...
  if (loading) return <div style={{ background: CARD, borderRadius: 12, padding: 16, border: `1px solid ${BORDER}`, color: TK.sub, fontSize: 12 }}>🔮 역-DCF 내재 기대 계산 중…</div>
  if (!d) return null
  const v = V[d.verdict]
  ...
  return (
    <div style={{ background: `${v.color}0d`, border: `1px solid ${v.color}55`, borderRadius: 14, padding: '15px 18px', display: 'flex', flexDirection: 'column', gap: 12 }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 9, flexWrap: 'wrap' }}>
        ...
        <span style={{ color: TK.slate200, fontWeight: 800, fontSize: 15 }}>역-DCF 기대치 투자</span>
        <span style={{ color: TK.sub2, fontSize: 11 }}>주가가 말하는 미래 — {name}</span>
      ...
      <div style={{ color: TK.slate200, fontSize: 13, fontWeight: 700, lineHeight: 1.6 }}>{d.headline}</div>

      {d.impliedGrowth != null && (
        <div style={{ background: TK.bg3, borderRadius: 10, padding: '12px 14px' }}>
          {/* 내재 기대 */}
          <Row label="📈 시장 내재 기대" val={d.impliedGrowth} pos={impPos} color={v.color} scale={SCALE} />
          {/* 실제/예상 성장 */}
          {d.actualGrowth != null && <div style={{ marginTop: 9 }}><Row label={d.growthSource === 'peg' ? '🎯 예상 성장(PEG 내재)' : '🏭 실제 성장(최근)'} val={d.actualGrowth} pos={actPos} color={TK.blue400} scale={SCALE} /></div>}
        </div>
      )}

      <div style={{ color: TK.sub5, fontSize: 11.5, lineHeight: 1.7 }}>{d.detail}</div>
...
function Row({ label, val, pos, color, scale }: { label: string; val: number; pos: number | null; color: string; scale: number }) {
  return (
    <div>
      ...
      <div style={{ height: 10, background: TK.border, borderRadius: 5, overflow: 'hidden', position: 'relative' }}>
        <div style={{ width: `${pos ?? 0}%`, height: '100%', background: color, borderRadius: 5 }} />
      </div>
      <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 1 }}>
        <span style={{ color: TK.slate600, fontSize: 8.5 }}>0%</span>
        <span style={{ color: TK.slate600, fontSize: 8.5 }}>{scale}%+ (10년 지속 극소수)</span>
      </div>
    </div>
  )
}
