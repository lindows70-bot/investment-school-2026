'use client'
// 학생 내 자산 '내 자산 흐름' 차트의 Recharts 부분 — GrowthChart 가 데이터를 받은 뒤에만 next/dynamic 으로 불러온다(recharts 청크를 첫 화면에서 뺀다)
...
/** month = 'YYYY-MM' · value = 월말 평가금액(valueKrw) · cost = 넣은 돈 = 그달 말 들고 있던 종목을 산 돈(valueKrw − cumPnl) · pnl = 불어난 돈(cumPnl) */
...
function Tip({ active, payload, lastMonth }: { active?: boolean; payload?: ReadonlyArray<{ payload?: GrowthDatum }>; lastMonth: string }) {
  ...
  return (
    <div style={{ background: TK.bg7, border: `1px solid ${TK.line1}`, borderRadius: RAD.sm, padding: SP.sm, display: 'flex', flexDirection: 'column', gap: SP.xs, fontSize: FS.tiny }}>
      ...
      <span style={{ color: C_VALUE }}>월말 평가금액 {won(d.value)}</span>
    ...
    <div style={{ width: '100%', height: 220, minWidth: 0 }}>
      <ResponsiveContainer width="100%" height="100%">
        <LineChart data={data} margin={{ top: SP.sm, right: SP.sm, bottom: 0, left: 0 }}>
          ...
          <Line type="monotone" dataKey="value" name="월말 평가금액" stroke={C_VALUE} strokeWidth={2} dot={false} isAnimationActive={false} />
        </LineChart>
      </ResponsiveContainer>
    </div>
  )
