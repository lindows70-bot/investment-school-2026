...
import { card, CardHead, toneColor, noteStyle, retryBtn, type IndexRow, type CalendarResp } from './homeUi'
...
/** 국내 장 상태 — KST 평일 09:00~15:30 = 장중. 휴장일 목록(/api/market-holidays)을 못 받으면 평일 규칙만 쓴다(그날이 공휴일이면 '장중'이 틀릴 수 있다) */
function krxStatus(now: number, holidays: string[] | null): string {
  ...
  if (day === 0 || day === 6 || holidays?.includes(ymd)) return '국내 휴장일'
...
export default function MarketBrief({ indices, calendar }: { indices: JsonResult<IndexRow[]>; calendar: JsonResult<CalendarResp> }) {
  const fx = useJson<{ rate?: unknown; source?: unknown }>('/api/exchange-rate')
  ...
  // 시각은 마운트 뒤에만 — 렌더 중 new Date() 는 서버(UTC)·브라우저(KST)가 다른 날을 봐 하이드레이션이 깨진다(전례)
  ...
  const today = now == null ? null : new Date(now + 9 * 3600_000).toISOString().slice(0, 10)
  ...
  //  (그 두 필드가 없는 옛 응답이면 전체 checked·failed 로 — 비트코인 실패가 '일부 확인 못 함'에 섞일 수 있다)
  const mChecked = isNum(md?.heldChecked) ? md.heldChecked : md?.checked
  const mFailed = isNum(md?.heldFailed) ? md.heldFailed : md?.failed
  ...
  return (
    <section style={{ ...card, display: 'flex', flexDirection: 'column', gap: SP.sm }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: SP.sm, flexWrap: 'wrap', paddingTop: SP.xs, borderTop: `1px solid ${TK.border}` }}>
        {failed.length > 0
          ? <button type="button" onClick={() => failed.forEach(s => s.reload())} style={retryBtn}>못 가져온 것 다시</button>
          : <span />}
