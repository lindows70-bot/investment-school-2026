...
export default function Greeting({ name }: { name: string | null | undefined }) {
  return (
    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: SP.sm, minHeight: 48 }}>
      ...
      <Link href="/dashboard" style={{ display: 'flex', alignItems: 'center', height: 44, flexShrink: 0, whiteSpace: 'nowrap', padding: `0 ${SP.md}px`, borderRadius: RAD.pill, border: `1px solid ${TK.border}`, color: TK.sub, fontSize: FS.tiny, textDecoration: 'none' }}>분석 화면</Link>
    </div>
  )
}
