...
const moatColor = (w: string) => w === 'wide' ? TK.green400 : w === 'moderate' ? TK.blue400 : w === 'narrow' ? TK.amber400 : TK.sub3
...
export default function MorningstarRatings() {
  ...
  if (loading) return <div style={{ background: CARD, borderRadius: 12, border: `1px solid ${BORDER}`, padding: '14px 16px', color: TK.sub, fontSize: 12 }}>🌟 모닝스타식 등급을 산정 중입니다…</div>
  ...
  return (
    <div style={{ background: CARD, borderRadius: 12, border: `1px solid ${BORDER}`, padding: '14px 16px' }}>
      <div style={{ display: 'flex', alignItems: 'baseline', gap: 8, flexWrap: 'wrap', marginBottom: 4 }}>
        <span style={{ color: TK.slate200, fontWeight: 800, fontSize: 13.5 }}>🌟 모닝스타식 스타 등급</span>
        {d.avgStars != null && <span style={{ color: TK.amber400, fontWeight: 800, fontSize: 12 }}>평균 ★{d.avgStars}</span>}
        <span style={{ color: TK.sub, fontSize: 11 }}>공정가치(DCF) × 해자 × 불확실성 × 자본배분</span>
      </div>
      <div style={{ color: TK.sub2, fontSize: 11, marginBottom: 11, lineHeight: 1.5 }}>
        ★5=공정가치 대비 크게 저평가 · ★1=고평가. <b style={{ color: TK.sub5 }}>불확실성이 높을수록 더 큰 안전마진(할인)을 요구</b>합니다 — 별점은 &lsquo;가치&rsquo;지 &lsquo;타이밍&rsquo;이 아닙니다.
      </div>
      ...
      {unrated.length > 0 && (
        <div style={{ marginTop: 10, paddingTop: 9, borderTop: `1px solid ${BORDER}` }}>
          <div style={{ color: TK.sub, fontSize: 10.5, marginBottom: 6 }}>⚠️ 공정가치(DCF) 산정 보류 — 적자·현금흐름 음수·데이터 부족, 또는 <b style={{ color: TK.amber400 }}>기저효과</b>(작년 이익 붕괴 후 폭증 → 공정가치 과대 착시)</div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
      ...
      <div style={{ marginTop: 11, color: TK.sub, fontSize: 9.5, lineHeight: 1.5 }}>
        ※ 모닝스타 공개 방법론(해자·공정가치·불확실성·자본배분)을 우리 DCF·해자 엔진으로 재현한 교육용입니다 — 모닝스타 공식 등급·적정가가 아닙니다. 공정가치=버핏식 보수적 DCF(현재 FCF·성장률 상한 35%)라 메가트렌드 성장주는 비싸게 나올 수 있습니다. <b style={{ color: TK.sub8 }}>경기순환주(에너지·반도체 등)는 현재 이익 기준 DCF라 사이클 위치에 따라 공정가치가 크게 왜곡될 수 있어 별점을 참고로만 보세요.</b> 투자 추천 아님.
      </div>
    </div>
  )
}
...
function Row({ e, open, onToggle }: { e: RatingEntry; open: boolean; onToggle: () => void }) {
  ...
  return (
    <div style={{ background: TK.bg3, border: `1px solid ${BORDER}`, borderRadius: 9 }}>
      <button onClick={onToggle} style={{ width: '100%', display: 'flex', alignItems: 'center', gap: 9, flexWrap: 'wrap', background: 'transparent', border: 'none', cursor: 'pointer', padding: '9px 12px', textAlign: 'left' }}>
        <span style={{ color: TK.slate200, fontWeight: 800, fontSize: 12.5, minWidth: 54 }}>{e.market === 'KR' ? (e.name || e.ticker).slice(0, 8) : e.ticker}</span>
