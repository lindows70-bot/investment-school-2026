...
const PH: Record<HcPhase, { name: string; color: string; desc: string }> = {
  2: { name: '② 호황기', color: TK.red400, desc: '가격↑ · 거래↑ — 상승 본격화' },
  3: { name: '③ 침체 진입', color: TK.orange400, desc: '가격↑ · 거래↓ — 비싸져 거래가 마름(상투 경계)' },
  4: { name: '④ 침체기', color: TK.yellow500, desc: '가격 보합 · 거래↓ — 관망' },
  5: { name: '⑤ 불황기', color: TK.blue400, desc: '가격↓ · 거래↓ — 조정 진행' },
  6: { name: '⑥ 회복 진입', color: TK.teal400, desc: '가격↓ · 거래↑ — 싸지자 거래가 살아남(바닥 탐색)' },
  1: { name: '① 회복기', color: TK.green400, desc: '가격 보합 · 거래↑ — 반등 준비' },
}
...
export default function HoneycombCycle() {
  ...
  if (err) return <div style={{ background: CARD, borderRadius: 12, padding: 24, border: `1px solid ${BORDER}`, color: TK.red400, fontSize: 12 }}>⚠️ 벌집순환 데이터를 불러오지 못했습니다 — 잠시 후 새로고침해주세요.</div>
  if (!d) return <div style={{ background: CARD, borderRadius: 12, padding: 24, border: `1px solid ${BORDER}`, color: TK.sub, fontSize: 12 }}>🐝 지역별 가격×거래량 국면을 계산 중… (한국부동산원 R-ONE, 첫 로드는 ~1분)</div>
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 7 }}>
        {([2, 3, 4, 5, 6, 1] as HcPhase[]).map(p => (
          <div key={p} style={{ flex: '1 1 150px', background: CARD, border: `1px solid ${d.phaseCount[p] ? PH[p].color : BORDER}`, borderRadius: 10, padding: '8px 12px' }}>
            <div style={{ color: PH[p].color, fontWeight: 800, fontSize: 11.5 }}>{PH[p].name} <span style={{ color: TK.slate200 }}>{d.phaseCount[p]}곳</span></div>
            <div style={{ color: TK.sub, fontSize: 10, marginTop: 2, lineHeight: 1.45 }}>{PH[p].desc}</div>
          </div>
        ))}
      </div>
      ...
      <div style={{ background: CARD, border: `1px solid ${BORDER}`, borderRadius: 14, padding: '16px 18px' }}>
        <div style={{ color: TK.slate200, fontWeight: 800, fontSize: 13 }}>🐝 벌집 시계 — 육각형 위의 현재 위치</div>
        <div style={{ color: TK.sub, fontSize: 11, margin: '3px 0 6px', lineHeight: 1.55 }}>
          고전 벌집모형 그대로 — 시장은 육각형을 <b style={{ color: TK.slate300 }}>시계방향</b>(②호황→③침체진입→④침체→⑤불황→⑥회복진입→①회복)으로 돕니다.
          ...
          return (
            <svg viewBox={`0 0 ${HW} ${HH}`} style={{ width: '100%' }}>
              ...
              {ORDER.map((_, i) => {
                ...
                return <text key={'ar' + i} x={mx} y={my + 4} textAnchor="middle" fill={TK.sub} fontSize={13} transform={`rotate(${ang} ${mx} ${my})`}>▶</text>
              })}
              {/* 꼭짓점(국면) + 지역 칩 */}
              {ORDER.map((ph, i) => {
                const x = vx(i), y = vy(i)
                        ...
                        {Array.from({ length: rows }, (_, row) => (
                          <text key={'r' + row} x={outX} y={outY + dir * (15 + row * 13)} textAnchor="middle" fontSize={10.5}>
                            {names.slice(row * 3, row * 3 + 3).map((nm, k) => (
                              <tspan key={nm} fill={nm === sel || nm === '🇰🇷전국' ? TK.slate100 : TK.sub5} fontWeight={nm === sel || nm === '🇰🇷전국' ? 900 : 600} onClick={() => nm !== '🇰🇷전국' && setSel(nm)} style={{ cursor: nm === '🇰🇷전국' ? 'default' : 'pointer' }}>{k > 0 ? ' · ' : ''}{nm}</tspan>
                            ))}
                          </text>
                        ))}
                      </>)
              ...
              {path.length > 1 && (
                <g>
                  <polyline points={path.map((s, k) => { const p = trailPt(s.ph, k, path.length); return `${p.x.toFixed(1)},${p.y.toFixed(1)}` }).join(' ')}
                    fill="none" stroke={TK.slate100} strokeWidth={1.6} strokeOpacity={0.6} strokeDasharray="4 3" />
                  {path.map((s, k) => {
                    const p = trailPt(s.ph, k, path.length)
                    const isLast = k === path.length - 1
                    return (
                      <g key={'tp' + k}>
                        <circle cx={p.x} cy={p.y} r={isLast ? 7 : 3.5} fill={isLast ? PH[s.ph].color : TK.slate300} stroke={isLast ? TK.slate100 : 'none'} strokeWidth={2} />
                        <text x={p.x + (isLast ? (p.x < cx ? 12 : -12) : 0)} y={p.y + (isLast ? 18 : -7)} textAnchor="middle" fill={isLast ? TK.slate100 : TK.sub} fontSize={isLast ? 10.5 : 8.5} fontWeight={isLast ? 900 : 600}>{isLast ? `${sel} 현재` : s.ym.slice(2)}</text>
                      </g>
                    )
                  })}
                </g>
      ...
      {/* 산점도 + 선택 지역 궤적 */}
      <div style={{ background: CARD, border: `1px solid ${BORDER}`, borderRadius: 14, padding: '16px 18px' }}>
        <div style={{ color: TK.slate200, fontWeight: 800, fontSize: 13 }}>📍 정밀 지도 — 실제 수치 좌표(가격×거래량)</div>
        <div style={{ color: TK.sub, fontSize: 11, margin: '3px 0 6px', lineHeight: 1.55 }}>
        ...
        <svg viewBox={`0 0 ${W} ${H}`} style={{ width: '100%' }}>
          ...
          <text x={W - padR} y={Y(0) - 6} textAnchor="end" fill={TK.sub} fontSize={10}>거래량 YoY(%) →</text>
          <text x={X(0) + 6} y={padT + 10} fill={TK.sub} fontSize={10}>가격 3개월(%) ↑</text>
          ...
          {pts.map(r => {
            ...
            return (
              <g key={r.name} style={{ cursor: 'pointer' }} onClick={() => setSel(r.name)}>
                <circle cx={x} cy={y} r={isSel ? 8 : isNat ? 7 : 5.5} fill={c} stroke={isSel || isNat ? TK.slate100 : TK.bg1} strokeWidth={isSel ? 2.2 : 1.2} />
                {(isSel || isNat || canLabel(x, y)) && (
                  <text x={x} y={y - (isSel ? 12 : 9)} textAnchor="middle" fill={isSel ? TK.slate100 : TK.sub5} fontSize={isSel || isNat ? 11 : 9.5} fontWeight={isSel || isNat ? 900 : 700}>{r.name}</text>
                )}
              </g>
            )
          })}
      ...
      <div style={{ background: CARD, border: `1px solid ${BORDER}`, borderRadius: 14, padding: '14px 18px' }}>
        <div style={{ color: TK.slate200, fontWeight: 800, fontSize: 13, marginBottom: 8 }}>지역별 국면 상세</div>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(215px,1fr))', gap: 7 }}>
          {d.regions.map(r => (
            <button key={r.name} onClick={() => setSel(r.name)} style={{
              textAlign: 'left', background: r.name === sel ? `${PH[r.phase].color}14` : TK.bg3, border: `1px solid ${r.name === sel ? PH[r.phase].color : BORDER}`,
              borderRadius: 9, padding: '8px 11px', cursor: 'pointer',
            }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline' }}>
                <b style={{ color: TK.slate200, fontSize: 12 }}>{r.name === '전국' ? '🇰🇷 전국' : r.name}</b>
                <span style={{ color: PH[r.phase].color, fontSize: 10.5, fontWeight: 800 }}>{PH[r.phase].name}</span>
              </div>
              <div style={{ color: TK.sub, fontSize: 10, marginTop: 3, fontFamily: 'monospace' }}>
      ...
      {/* 교육 + 캐비엇 */}
      <div style={{ background: CARD, border: `1px solid ${BORDER}`, borderRadius: 14, padding: '14px 18px', fontSize: 11, color: TK.sub5, lineHeight: 1.7 }}>
        <b style={{ color: TK.slate200 }}>🎓 벌집모형 읽는 법</b> — 부동산은 공급이 느려(인허가→입주 2~3년) 가격과 거래량이 벌집(육각형)을 시계방향으로 도는 경향이 있습니다.
        핵심 신호 2개: <b style={{ color: TK.orange400 }}>③ 침체 진입(가격은 오르는데 거래가 마름)</b> = 추격 매수 경계 /
        <b style={{ color: TK.teal400 }}> ⑥ 회복 진입(가격은 빠지는데 거래가 살아남)</b> = 바닥 탐색 신호(스마트머니 선진입).
        <div style={{ marginTop: 6, color: TK.sub, fontSize: 10.5 }}>
