  ...
  return (
    <div style={{ marginBottom: 12 }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4, fontSize: 12.5 }}>
        ...
        <span style={{ marginLeft: 'auto', color: TK.sub2, fontSize: 10.5 }}>20 과매도 · 80 과매수</span>
      </div>
      ...
      {us.instPct != null && (
        <>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, margin: '12px 0 4px', fontSize: 12.5 }}>
            ...
            {us.instCount != null && <span style={{ color: TK.sub2, fontSize: 11 }}>{us.instCount.toLocaleString()}곳</span>}
            {us.instTrend && (
              <span style={{ marginLeft: 'auto', fontSize: 11, fontWeight: 700, color: us.instTrend === 'accum' ? TK.emerald400 : us.instTrend === 'distrib' ? TK.red400 : TK.sub }}>
          ...
          <div style={{ color: TK.sub2, fontSize: 10, marginTop: 3 }}>Top 기관의 분기 지분 증감(13F·45일 지연) — 9인 거장보다 넓은 스마트머니</div>
        </>
      )}
      {/* 내부자 · 13F 칩 */}
      <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginTop: 10 }}>
        <span title="EDGAR Form 4의 공개시장 매수(코드 P) — 린치가 말한 '내부자가 사는 이유는 하나'인 진짜 매수 신호" style={{ background: us.insiderBuyers > 0 ? 'rgba(34,197,94,0.12)' : TK.bg3, color: us.insiderBuyers > 0 ? TK.emerald400 : TK.sub, border: `1px solid ${us.insiderBuyers > 0 ? `${TK.green500}44` : TK.border}`, borderRadius: 8, padding: '4px 11px', fontSize: 12, fontWeight: 600 }}>
...
export default function MoneyFlowRadar({ ticker, name, market }: { ticker: string; name: string; market: string }) {
  ...
  if (loading) {
    return (
      <div style={{ background: CARD, borderRadius: 12, padding: 20, border: `1px solid ${BORDER}`, color: TK.sub, fontSize: 13 }}>
        💰 수급 흐름을 분석 중입니다…
      </div>
    )
  }
    ...
    return (
      <div style={{ background: CARD, borderRadius: 12, padding: 18, border: `1px solid ${BORDER}` }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6 }}>
          ...
          <span style={{ color: TK.slate200, fontWeight: 700, fontSize: 15 }}>스마트머니 수급 레이더</span>
        </div>
        <div style={{ color: TK.sub, fontSize: 12.5, lineHeight: 1.6 }}>{data?.note ?? '수급 데이터를 제공할 수 없는 종목입니다.'}</div>
      </div>
    )
  }
  ...
  return (
    <div style={{ background: CARD, borderRadius: 12, padding: '16px 20px', border: `1px solid ${data.status === 'INFLOW' ? `${TK.green500}55` : data.status === 'CROWDED' ? `${TK.red500}55` : BORDER}` }}>
      ...
      <div style={{ color: TK.sub2, fontSize: 11.5, marginBottom: 12 }}>
        {data.us
          ...
          {actors.map(({ key, label, a }) => {
            ...
            return (
              <div key={key} style={{ display: 'flex', alignItems: 'center', gap: 10, fontSize: 12.5 }}>
                ...
                <span style={{ width: 64, textAlign: 'right', color: a?.dir === 'FLAT' ? TK.slate500 : buy ? TK.emerald400 : TK.red400, fontSize: 11, flexShrink: 0 }}>
      ...
      {data.badges.length > 0 && (
        <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginBottom: 10 }}>
          {data.badges.map((b, i) => (
            <span key={i} style={{ background: TK.border, color: TK.slate300, border: `1px solid ${BORDER}`, borderRadius: 8, padding: '3px 9px', fontSize: 11.5, fontWeight: 600 }}>{b}</span>
          ))}
        </div>
      )}
      ...
      <div style={{ background: `${cfg.color}0d`, border: `1px solid ${cfg.color}33`, borderRadius: 10, padding: '11px 14px' }}>
        <div style={{ color: TK.slate300, fontSize: 12.5, lineHeight: 1.65 }}>{data.lynchComment}</div>
