...
export default function FxAttributionCard() {
  ...
  if (loading) return <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 14, padding: 18, fontSize: FS.tiny, color: C.low }}>💱 환율 기여도 계산 중…</div>
  ...
  return (
    <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 14, padding: '16px 18px' }}>
      <div style={{ display: 'flex', alignItems: 'baseline', gap: 10, flexWrap: 'wrap' }}>
        <span style={{ fontSize: FS.body, fontWeight: 900, color: C.text }}>💱 환율 기여도</span>
        <span style={{ fontSize: FS.tiny, color: C.sub }}>달러로 번 것 vs 환율이 만든 것</span>
        <span style={{ marginLeft: 'auto', fontSize: FS.tiny, color: C.low, fontFamily: 'monospace' }}>현재 ₩{d.fxNow.toLocaleString()}</span>
      </div>
      ...
      <div style={{ marginTop: 11, background: '#141b26', border: `1px solid ${C.cyan}33`, borderRadius: 10, padding: '11px 13px' }}>
        <div style={{ fontSize: FS.tiny, fontWeight: 900, color: C.text }}>🧮 미국 주식은 <span style={{ color: C.cyan }}>두 번</span> 계산됩니다</div>
        <div style={{ fontSize: FS.tiny, color: C.sub, marginTop: 5, lineHeight: 1.7 }}>
        ...
        {[
          ...
          <div key={x.k} style={{ display: 'flex', alignItems: 'center', gap: 10, flex: '1 1 160px' }}>
            <div style={{ flex: 1, background: x.n === '③' ? '#161d18' : C.card2, border: `1px solid ${x.n === '③' ? C.green + '44' : C.border}`, borderRadius: 10, padding: '10px 12px' }}>
              <div style={{ fontSize: FS.tiny, color: C.low }}>
                <span style={{ color: x.n === '②' ? C.gold : C.cyan, fontWeight: 900 }}>{x.n}</span> {x.k}
              </div>
              <div style={{ fontSize: FS.xl, fontWeight: 900, fontFamily: 'monospace', color: x.c }}>{x.v}</div>
              <div style={{ fontSize: FS.tiny, color: C.low, marginTop: 1 }}>{x.sub}</div>
            </div>
            {x.op && <span style={{ fontSize: FS.lg, fontWeight: 900, color: C.low, flexShrink: 0 }}>{x.op}</span>}
          </div>
        ))}
      </div>
      <div style={{ fontSize: FS.tiny, color: C.sub, marginTop: 7, lineHeight: 1.6 }}>
      ...
      {flipped.length > 0 && (
        <div style={{ marginTop: 9, fontSize: FS.tiny, fontWeight: 700, color: C.gold, background: '#2a2213', borderRadius: 8, padding: '8px 11px', lineHeight: 1.6 }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 5, marginTop: 11 }}>
        <div style={{ display: 'flex', alignItems: 'baseline', gap: 9, padding: '0 11px 2px', borderBottom: `1px solid ${C.border}` }}>
          <span style={{ fontSize: FS.tiny, fontWeight: 700, color: C.low }}>종목 · 매입 환율</span>
          <span style={{ marginLeft: 'auto', display: 'flex', gap: 8, alignItems: 'baseline', fontSize: FS.tiny, fontWeight: 700 }}>
        ...
        {shown.map(r => (
          <div key={r.ticker} style={{ display: 'flex', alignItems: 'center', gap: 9, flexWrap: 'wrap', background: r.flipped ? '#221c10' : C.card2, borderRadius: 8, padding: '7px 11px', borderLeft: `3px solid ${col(r.fxContrib)}` }}>
            <span style={{ fontSize: FS.tiny, fontWeight: 800, color: C.text }}>{r.name.length > 18 ? r.name.slice(0, 18) : r.name}</span>
            <span style={{ fontSize: FS.tiny, fontFamily: 'monospace', color: C.low }}>{r.ticker}</span>
            <span style={{ fontSize: FS.tiny, color: C.low }} title={`매입 ${r.purchaseDate}${r.lots > 1 ? ` 외 ${r.lots - 1}건(원가 가중)` : ''}`}>
              매입 ₩{r.fxBuy.toLocaleString()}{r.lots > 1 && ` (${r.lots}건)`}
            </span>
            <span style={{ marginLeft: 'auto', display: 'flex', gap: 8, alignItems: 'baseline', fontFamily: 'monospace', fontSize: FS.tiny }}>
              <span style={{ minWidth: 52, textAlign: 'right', color: col(r.retUsd) }} title="① 달러 기준 — 종목이 실제로 번 것">{pct(r.retUsd)}</span>
              <span style={{ minWidth: 10, textAlign: 'center', color: C.low, fontSize: FS.tiny }}>+</span>
              <span style={{ minWidth: 56, textAlign: 'right', color: col(r.fxContrib), fontWeight: 800 }} title="② 환율 기여 — 환율이 만든 것">{pp(r.fxContrib)}</span>
              <span style={{ minWidth: 10, textAlign: 'center', color: C.low, fontSize: FS.tiny }}>=</span>
              <span style={{ minWidth: 58, textAlign: 'right', color: col(r.retKrw), fontWeight: 900, fontSize: FS.tiny }} title="③ 원화 기준 — 내 계좌에 찍히는 것">{pct(r.retKrw)}</span>
            </span>
          </div>
        ))}
        {d.rows.length > 5 && (
          <div onClick={() => setOpenAll(!openAll)} style={{ fontSize: FS.tiny, color: C.cyan, cursor: 'pointer', textAlign: 'center', padding: '3px 0' }}>
      ...
      <div style={{ marginTop: 12, background: C.card2, borderRadius: 10, padding: '10px 12px' }}>
        <div style={{ fontSize: FS.tiny, fontWeight: 800, color: C.text }}>환율만 움직인다면 (다른 조건 그대로)</div>
        <div style={{ display: 'flex', gap: 7, flexWrap: 'wrap', marginTop: 7 }}>
          {d.scenarios.map(s => (
            <span key={s.deltaPct} style={{ fontSize: FS.tiny, background: TK.bg3, borderRadius: 6, padding: '4px 9px', color: C.sub }}>
      ...
      <div style={{ fontSize: FS.tiny, color: C.low, marginTop: 10, lineHeight: 1.55 }}>
