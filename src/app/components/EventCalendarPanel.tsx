...
export default function EventCalendarPanel({ compact }: { compact?: boolean }) {
    ...
    return (
      <div style={{ background: TK.bg1, border: `1px solid ${TK.border}`, borderRadius: 12, padding: '12px 15px' }}>
        <div style={{ display: 'flex', alignItems: 'baseline', gap: 8, marginBottom: 8 }}>
          <span style={{ fontSize: FS.tiny, fontWeight: 800, color: TK.amber400 }}>📅 이번 주 내 종목 이벤트 {week.length}건</span>
          <a href="/assets" style={{ fontSize: FS.tiny, color: TK.sub4, textDecoration: 'none' }}>자산 관리에서 전체 캘린더 →</a>
        </div>
        <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', alignItems: 'center' }}>
        {week.map((e, i) => {
          const m = EV_META[e.type]
          return (
            <span key={i} style={{ display: 'inline-flex', alignItems: 'center', gap: 5, background: TK.bg2, border: `1px solid ${m.color}44`, borderRadius: 7, padding: '3px 9px', fontSize: FS.tiny, whiteSpace: 'nowrap' }}>
              <b style={{ color: m.color, fontSize: FS.tiny }}>{e.dDay === 0 ? 'TODAY' : `D-${e.dDay}`}</b>
              <span>{m.icon}</span>
  ...
  if (err) return <div style={{ ...CARD, color: TK.sub4, fontSize: FS.tiny }}>이벤트 캘린더를 불러오지 못했습니다.</div>
  if (!data) return <div style={{ ...CARD, color: TK.sub4, fontSize: FS.tiny }}>📅 보유 종목의 실적·배당 일정을 수집하는 중…</div>
  ...
  return (
    <div style={{ ...CARD, display: 'flex', flexDirection: 'column', gap: 14 }}>
      <div style={{ display: 'flex', alignItems: 'baseline', gap: 10, flexWrap: 'wrap' }}>
        <span style={{ fontSize: FS.lg, fontWeight: 800, color: TK.slate200 }}>📅 내 종목 이벤트 캘린더</span>
        <span style={{ fontSize: FS.tiny, color: TK.sub4 }}>향후 90일 실적·배당 일정 + 배당 현금흐름 · {data.scanned}종목</span>
      </div>

      {/* ① 타임라인 */}
      {data.events.length === 0 ? (
        <div style={{ fontSize: FS.tiny, color: TK.sub4 }}>향후 90일 내 예정된 실적·배당 이벤트가 없습니다{data.krNoEarnings ? ' (일부 한국 종목은 실적일 미제공)' : ''}.</div>
      ) : (
      ...
      {hasDiv && (
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: 12 }}>
          <div>
            <div style={{ fontSize: FS.tiny, color: TK.sub4 }}>연간 예상 배당 (보유수량 × 연배당)</div>
            <div style={{ fontSize: FS.h2, fontWeight: 900, color: TK.green400, margin: '2px 0 8px' }}>{fmtW(data.annualDivKrw)}</div>
          ...
          <div>
            <div style={{ fontSize: FS.tiny, color: TK.sub4, marginBottom: 6 }}>월별 예상 배당 (지난 1년 지급 패턴 투영·추정)</div>
            <div style={{ display: 'flex', alignItems: 'flex-end', gap: 4, height: 90 }}>
              {data.monthly.map(m => (
                <div key={m.month} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 3 }}>
                  {m.krw > 0 && <span style={{ fontSize: FS.tiny, color: TK.sub4 }}>{m.krw >= 1e4 ? `${(m.krw / 1e4).toFixed(m.krw < 1e5 ? 1 : 0)}만` : `${Math.round(m.krw / 1e3)}천`}</span>}
                  ...
                  <span style={{ fontSize: FS.tiny, color: TK.sub2 }}>{Number(m.month.slice(5))}월</span>
                </div>
              ))}
            </div>
          </div>
