...
const C_KITCHIN = TK.blue400, C_JUGLAR = TK.emerald400, C_KOND = TK.amber500, C_SUM = TK.blue300
// 각 파동 주기(년)
...
export default function EconomicWaveSimulator() {
  ...
  const phase = useMemo(() => {
    ...
    if (rising && last >= 0) return { label: '성장 및 확장 국면', color: TK.emerald400, desc: '세 파동이 상승 동조 — 호황 정점을 향하는 구간' }
    if (rising && last < 0) return { label: '회복 국면', color: TK.blue400, desc: '바닥을 다지고 반등하는 구간(저점 탈출)' }
    if (!rising && last >= 0) return { label: '둔화 국면', color: TK.amber500, desc: '고점을 통과해 상승 동력이 식는 구간' }
    return { label: '침체 국면', color: TK.red400, desc: '하강 파동이 겹쳐 위축되는 구간' }
  }, [data])
  ...
  const Slider = ({ label, sub, v, set, color }: { label: string; sub: string; v: number; set: (n: number) => void; color: string }) => (
    <div style={{ display: 'flex', alignItems: 'center', gap: 10, flex: '1 1 240px' }}>
      ...
      <div style={{ flex: 1 }}>
        <div style={{ color: TK.sub11, fontSize: 11, fontWeight: 700 }}>{label}</div>
        <div style={{ color: TK.sub2, fontSize: 9.5 }}>{sub}</div>
      </div>
      <input type="range" min={0} max={10} step={1} value={v} onChange={e => set(Number(e.target.value))}
        style={{ width: 110, accentColor: color }} />
      <span style={{ width: 26, textAlign: 'center', color: TK.slate200, fontWeight: 800, fontSize: 13, fontFamily: 'monospace' }}>{v}</span>
    </div>
  )
  ...
  return (
    <div style={{ background: CARD, borderRadius: 12, border: `1px solid ${BORDER}`, padding: '14px 16px' }}>
      <div style={{ display: 'flex', alignItems: 'baseline', gap: 8, flexWrap: 'wrap', marginBottom: 8 }}>
        <span style={{ color: TK.slate200, fontWeight: 800, fontSize: 13 }}>🌊 경제 파동 중첩 시뮬레이터</span>
        <span style={{ color: TK.sub, fontSize: 10.5 }}>키친·주글라르·콘드라티예프 3대 파동의 합 = 실제 경기</span>
      </div>
        ...
        <ResponsiveContainer width="100%" height="100%">
          <LineChart data={data} margin={{ top: 10, right: 16, left: 0, bottom: 4 }}>
            ...
            <XAxis dataKey="t" type="number" domain={[0, YEARS]} ticks={[0, 10, 20, 30, 40, 50, 60]}
              tick={{ fill: TK.sub2, fontSize: 9.5 }} axisLine={{ stroke: BORDER }} tickLine={false}
              label={{ value: '시간 (년) →', position: 'insideBottomRight', offset: -2, fill: TK.sub2, fontSize: 9.5 }} />
            <YAxis tick={{ fill: TK.sub2, fontSize: 9.5 }} axisLine={false} tickLine={false} width={34}
              label={{ value: '경기 지수 ↑', position: 'insideTopLeft', fill: TK.sub2, fontSize: 9.5, dy: -2 }} />
      ...
      <div style={{ textAlign: 'center', margin: '6px 0 10px' }}>
        <div style={{ color: TK.sub, fontSize: 11 }}>시장 종합 추세</div>
        <div style={{ color: phase.color, fontWeight: 800, fontSize: 15 }}>{phase.label}</div>
        <div style={{ color: TK.sub2, fontSize: 10.5 }}>{phase.desc}</div>
      </div>
      ...
      <button onClick={() => setEduOpen(o => !o)} style={{ width: '100%', display: 'flex', alignItems: 'center', gap: 8, background: 'transparent', border: 'none', cursor: 'pointer', padding: '10px 0 2px', textAlign: 'left' }}>
        <span style={{ color: TK.btcOrange, fontWeight: 800, fontSize: 12 }}>🎓 왜 4계절(거시)과 현실(AI)이 어긋날까?</span>
      ...
      {eduOpen && (
        <div style={{ color: TK.sub5, fontSize: 11, lineHeight: 1.65, marginTop: 6, display: 'flex', flexDirection: 'column', gap: 8 }}>
          ...
          <div style={{ background: 'rgba(96,165,250,0.07)', border: '1px solid rgba(96,165,250,0.25)', borderRadius: 8, padding: '8px 11px' }}>
            🧭 <b style={{ color: TK.blue300 }}>핵심</b> — 4계절 내비게이터는 <b>단·중기 파동(키친·주글라르)</b>을 측정합니다. 그런데 지금 시장은 <b>콘드라티예프 상승 파동(AI)</b>이라는 거대한 해일 위에 있어, &ldquo;여름이라 기술주 불리&rdquo;라는 교과서를 무시하고 오릅니다. <b>괴리는 버그가 아니라, 측정 중인 파동보다 더 큰 파동이 시장을 끌고 가고 있다는 신호</b>입니다.
          </div>
          <div style={{ color: TK.sub2 }}>⚠️ 위 차트는 파동 원리를 보여주는 <b>교육용 사인파 모델</b>(실측 경기 데이터 아님)입니다. 진폭을 바꿔보며 &lsquo;큰 파동이 작은 파동을 어떻게 압도하는지&rsquo;를 직접 확인하세요.</div>
        </div>
      )}
    </div>
  )
