...
export default function TradePlanCard({ market, timing, portfolioKrw }: {
  ...
  const v = qty <= 0
    ? { tone: TK.slate400, title: '⏸️ 지금은 관망하세요', sub: '이 리스크 예산으로는 1주도 담기 부담됩니다 — 리스크%를 올리거나 더 싼 종목을 찾으세요.', now: 0 }
    : t.light === 'green'
    ? { tone: TK.green500, title: '✅ 지금 사도 됩니다 (한 번에 말고 나눠서)', sub: `추세가 살아있는 자리입니다. 먼저 ${first}주만 담고, 눌릴 때(${dipLabel} ${fmtP(dip)})에 추가, 반등 확인 후 나머지를 채우세요.`, now: first }
    : t.light === 'yellow'
    ? { tone: TK.yellow500, title: '⏳ 절반만 지금, 나머지는 기다리세요', sub: `아직 방향 확정 전입니다. 지금은 ${half}주까지만 담고, 구름 상단(${fmtP(t.cloudTop)}) 돌파를 확인한 뒤 나머지 절반을 채우세요.`, now: half }
    : { tone: TK.red500, title: '🚫 지금은 사지 마세요', sub: `진입을 미룰 자리입니다. 굳이 담고 싶다면 구름 상단(${fmtP(t.cloudTop)}) 돌파를 확인한 뒤 소액부터.`, now: 0 }
          ...
          {detail && (
            <div style={{ marginTop: 8 }}>
              ...
              <div style={{ background: TK.bg3, borderRadius: 8, padding: '9px 11px', marginBottom: 8, display: 'grid', gridTemplateColumns: 'auto 1fr', columnGap: 12, rowGap: 3, fontSize: 10.5 }}>
                ...
                <span style={{ color: TK.sub5, fontFamily: 'monospace' }}>감당손실 {fmtW(riskBudgetKrw)} ÷ 주당리스크 = <b style={{ color: TK.green400 }}>{qty.toLocaleString()}주</b></span>
                <span style={{ color: TK.sub2 }}>익절 참고선</span>
                <span style={{ color: TK.amber400, fontFamily: 'monospace' }}>2R {fmtP(r2)} · 3R {fmtP(r3)} <span style={{ color: TK.sub2, fontFamily: 'inherit' }}>(손실 1 벌 때 2~3 먹는 손익비)</span></span>
              </div>
              ...
              {t.raschke && (() => {
                ...
                const stageMap: Record<number, { t: string; c: string; g: string }> = {
                  0: { t: '연쇄 시작 전', c: TK.sub3, g: '모멘텀 전환 신호 아직 없음 — 서두르지 말고 관망.' },
                  1: { t: 'CCI 신호탄(선행)', c: TK.yellow500, g: '바닥권 탈출 신호탄 — 성급한 진입보다 RSI 50 돌파(에너지)를 먼저 확인.' },
                  2: { t: 'RSI 50 돌파(에너지)', c: TK.yellow500, g: '매수세가 붙는 중 — MACD 영선 돌파(추세 확정)까지 기다리면 확률↑.' },
                  3: { t: 'MACD 영선 돌파(추세 확정)', c: TK.green400, g: '추세 확정 — 첫 눌림목(숨 고르기)이 오면 그때가 최적 1차 진입.' },
                }
                ...
                return (
                  <div style={{ background: `${col}0d`, border: `1px solid ${col}44`, borderRadius: 8, padding: '7px 10px', marginBottom: 6, fontSize: 10.5, lineHeight: 1.55 }}>
                    ...
                    <span style={{ color: TK.sub5 }}>
                      ...
                      {chop && <>. <b style={{ color: TK.slate300 }}>추세 강도 약함(ADX {s.adx})</b> — 돌파 신호가 나와도 <b style={{ color: TK.slate200 }}>가짜 돌파(휩쏘)</b>일 수 있어 방향 확정 후 진입.</>}
                      {strong && <>. 매수자·거래 대다수가 현재가 아래 = <b style={{ color: '#7dd3fc' }}>눌림 지지 확보</b>.</>}
                      {extended && <>. 평단·매물대가 <b style={{ color: TK.amber400 }}>크게 아래 = 지지선 멀다</b> — 추격보다 되돌림·분할 진입.</>}
                      {weak && <>. 평단·매물 대다수가 위 = <b style={{ color: '#fdba74' }}>지지 얇음</b> — 반등·매물 소화 확인 후.</>}
                    </span>
                    {s.fvgBuyHi != null && (
                      <div style={{ color: TK.lime400, marginTop: 3 }}>
