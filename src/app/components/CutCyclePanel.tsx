...
import type { CutCycleResult } from '@/lib/cutCycleHistory'
...
export default function CutCyclePanel({ d }: { d: CutCycleResult }) {
  ...
  const falseAlarms = d.cycles.filter(c => c.kind === 'insurance' && c.recessionWithin12m != null)
    ...
    <div style={box}>
          ...
          <div key={s.kind} style={{ background: TK.bg0, borderRadius: RAD.sm, padding: '10px 13px', borderLeft: `3px solid ${s.kind === 'insurance' ? TK.green400 : TK.red400}` }}>
            ...
            {s.suppressed ? (
              ...
              <div style={{ display: 'flex', gap: SP.md, flexWrap: 'wrap' }}>
                ...
                <div><div style={{ fontSize: FS.micro, color: TK.sub4 }}>12개월 내 침체</div><b style={{ fontSize: FS.lg, color: s.recessionRate != null && s.recessionRate >= 30 ? TK.amber400 : TK.slate200, fontFamily: 'monospace' }}>{s.recessionRate != null ? s.recessionRate + '%' : '—'}</b></div>
              </div>
            )}
          </div>
        ))}
      ...
      {ins && ins.medSpx12 != null && (
        <div style={{ padding: '10px 13px', borderRadius: RAD.sm, background: `${TK.amber400}0e`, border: `1px solid ${TK.amber400}33`, fontSize: FS.micro, color: TK.sub2, lineHeight: 1.75, marginBottom: SP.md }}>
          ...
          {' '}<b style={{ color: TK.slate200 }}>다만 {ins.n}건 중 {falseAlarms.length}건({ins.recessionRate}%)은 1년 안에 침체가 왔습니다</b>
          {falseAlarms.length > 0 && <> — {falseAlarms.map(f => f.start.slice(0, 7)).join(' · ')}. 당시에도 &ldquo;경기는 괜찮다&rdquo;고 봤지만 아니었습니다.</>}
        ...
        <table style={{ width: '100%', minWidth: 620, borderCollapse: 'collapse', fontSize: FS.micro }}>
          <thead>
            <tr style={{ color: TK.sub4 }}>
              {['인하 시작', '정책금리', '실업률(12M변화)', '유형', '12개월 내 침체', 'S&P 6M', '12M', '24M', '10년물 12M'].map(h => (
          ...
          <tbody>
            {d.cycles.slice().reverse().map(c => (
              <tr key={c.start} title={c.kindNote}>
                ...
                <td style={{ padding: '5px 6px', textAlign: 'right', color: c.recessionWithin12m ? TK.amber400 : TK.sub4, fontFamily: 'monospace' }}>{c.recessionWithin12m?.slice(0, 7) ?? '—'}</td>
