...
const ROLE_COLOR: Record<string, string> = {
  CORE_INDEX: '#3b82f6', CORE_BOND: '#22d3ee', SATELLITE_BTC: '#f59e0b',
  SATELLITE_GHOST: '#a78bfa', SATELLITE_GENERAL: '#22c55e', BLOCKED: '#ef4444',
}
  ...
  return (
    <div style={{ flex: '1 1 150px', minWidth: 140 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 3 }}>
        ...
        <span style={{ color: over ? '#ef4444' : '#cbd5e1', fontSize: 11.5, fontWeight: 800, fontFamily: 'monospace' }}>{pct}% / 캡 {cap}%</span>
      </div>
      <div style={{ height: 7, background: '#0f1117', borderRadius: 4, overflow: 'hidden', position: 'relative' }}>
        <div style={{ width: `${w}%`, height: '100%', background: over ? '#ef4444' : color, borderRadius: 4 }} />
        <div style={{ position: 'absolute', left: `${(cap / (cap * 1.6)) * 100}%`, top: 0, bottom: 0, width: 1.5, background: '#e2e8f0' }} />
      </div>
    </div>
  )
}
  ...
  return (
    <div style={{ flex: '1 1 280px', minWidth: 260, background: CARD, borderRadius: 12, border: `1px solid ${color}44`, padding: '12px 14px' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 7, marginBottom: 9 }}>
        ...
        <span style={{ marginLeft: 'auto', color: '#64748b', fontSize: 12, fontWeight: 700 }}>{count}</span>
      </div>
      {count === 0
        ? <div style={{ color: '#64748b', fontSize: 11.5, padding: '6px 0' }}>해당 없음 ✓</div>
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
      ...
      <div style={{ background: CARD, borderRadius: 12, border: `1px solid ${BORDER}`, padding: '14px 16px' }}>
        <div style={{ display: 'flex', alignItems: 'baseline', gap: 8, marginBottom: 9, flexWrap: 'wrap' }}>
          <span style={{ color: '#e2e8f0', fontWeight: 800, fontSize: 14 }}>🎯 자산군 구성 (코어-새틀라이트)</span>
          <span style={{ color: '#7f93a8', fontSize: 11 }}>전 자산 기준 · 캡: BTC·유령 각 {cs.capPct}%</span>
        </div>
        <div style={{ display: 'flex', height: 22, borderRadius: 6, overflow: 'hidden', marginBottom: 7 }}>
          {cs.groups.map(g => <div key={g.role} title={`${g.label} ${g.pct}%`} style={{ width: `${g.pct}%`, background: ROLE_COLOR[g.role] ?? '#64748b' }} />)}
        </div>
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: '4px 12px', marginBottom: 11 }}>
          {cs.groups.map(g => (
            <span key={g.role} style={{ display: 'inline-flex', alignItems: 'center', gap: 4, fontSize: 10.5, color: '#94a3b8' }}>
              <span style={{ width: 8, height: 8, borderRadius: 2, background: ROLE_COLOR[g.role] ?? '#64748b' }} />
              {g.label} <b style={{ color: '#e2e8f0', fontFamily: 'monospace' }}>{g.pct}%</b>
            </span>
          ))}
        </div>
        <div style={{ display: 'flex', gap: 14, flexWrap: 'wrap' }}>
          <div style={{ flex: '1 1 180px', minWidth: 160 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 3 }}>
              <span style={{ color: '#94a3b8', fontSize: 11 }}>코어 (목표 {cs.coreTargetMin}~{cs.coreTargetMax}%)</span>
              <span style={{ color: coreOk ? '#22c55e' : '#f59e0b', fontSize: 11.5, fontWeight: 800, fontFamily: 'monospace' }}>{cs.corePct}% {coreOk ? '적정' : cs.corePct < cs.coreTargetMin ? '부족' : '과다'}</span>
            </div>
            <div style={{ height: 7, background: '#0f1117', borderRadius: 4, overflow: 'hidden', position: 'relative' }}>
              ...
              <div style={{ width: `${Math.min(cs.corePct, 100)}%`, height: '100%', background: coreOk ? '#22c55e' : '#f59e0b', borderRadius: 4 }} />
            </div>
            <div style={{ color: '#64748b', fontSize: 9.5, marginTop: 2 }}>{cs.coreTargetText}</div>
          </div>
      ...
      <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap' }}>
        <ActionCard icon="🗑️" title="버릴 것" color="#ef4444" count={cs.drop.length}>
        ...
        <ActionCard icon="✂️" title="줄일 것" color="#f59e0b" count={cs.trim.length}>
        ...
        <ActionCard icon="🛒" title="보강할 것" color="#22c55e" count={cs.add.length}>
