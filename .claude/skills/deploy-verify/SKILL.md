...
### 3-b. 클라우드 세션(claude.ai/code)에서는 — 브랜치 → PR → 직접 머지 → 자동 배포 확인 (2026-09-24 확정)

클라우드 컨테이너엔 Vercel 로그인이 없어 위 CLI 배포를 못 한다. 대신 **Vercel–GitHub 연동이 main 푸시를 프로덕션으로 자동 배포**한다(실측: PR #1 머지 커밋 `0d8ca0d` 가 머지 2분 뒤 "Deployment has completed", 프로덕션 응답이 새 코드로 바뀜).
1. 브랜치에서 작업 → `check:build` 통과 → 푸시 → 드래프트 PR(프리뷰 배포가 자동으로 붙는다).
2. 프리뷰 Vercel 상태 success + 충돌 없음이면 **사용자에게 묻지 말고 직접 머지한다** — 사용자 지시("내 머지 결정을 기다리는 건 또 뭐야? 니가 다 알아서 해!"). 드래프트 해제는 `POST /repos/{o}/{r}/pulls/{n}/ccr/ready_for_review`, 머지는 `PUT …/pulls/{n}/merge`(GraphQL 은 세션에서 막힘).
3. 머지 뒤 main 커밋의 Vercel 상태가 success 가 될 때까지 폴링하고, 프로덕션 API 로 **새 코드가 실제로 서빙되는지**(바뀐 문구·필드) 확인한다. 이게 이 경로의 `git log -1` 이다.
4. 세션 토큰은 브랜치 삭제(`DELETE git/refs`)가 403 이라 머지된 브랜치는 남는다 — 사용자가 GitHub 의 Delete branch 로 지우거나 그냥 둔다.
- ⚠️ 프로덕션 호스트가 프록시 정책에 막혀 있으면(HTTP 000) 환경 설정의 Network access 를 **전체**로 바꿔야 한다(설정 → 메시지 입력창 위 `Default` 환경 → ⚙). 2026-09-24 에 사용자가 바꿔 두었다.
