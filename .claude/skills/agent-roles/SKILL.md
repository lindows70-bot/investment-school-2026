...
### Gemini 실측 제약 (2026-07-28)
- **무료 티어 하루 ~20회.** `gemini-2.5-pro` 는 `limit: 0` 으로 **호출 불가**(Flash 계열만). `models.list` 에 pro 가 1M 컨텍스트로 보이는 건 '내 티어가 부를 수 있다'는 뜻이 **아니다** — 가용성은 실제 호출로 확인할 것.
- **에이전트 방식(gemini-cli 가 스스로 파일 탐색)은 이 한도에서 불가.** 왕복 1회 = 요청 1회라 감사 한 건에 수십 회를 쓴다. 그래서 `gemini-audit.mjs` 는 **수집은 로컬 grep, 모델은 판단만, 요청 1회** 구조다. 이 설계를 되돌리지 마라.
- 앱의 `GEMINI_API_KEY` 를 공유하므로 남발하면 주간 리포트·뉴스 레이더가 굶는다(앱은 주로 `flash-lite` 라 한도가 갈리긴 한다).
- `cache-keys` 모드는 기계적 후보가 0이면 **API 호출 없이 종료**한다(한도 절약).

### 첫 감사가 잡은 것(효용 근거)
`marks-cycle` writer v4 ↔ unified-reco reader v3 → 캐시 영구 미스로 **FCF 방어 틸트가 2주간 죽어 있었다**. `market-flow-kr` 은 크론이 v5 를 데우는데 화면은 v10 을 읽어 **크론 워밍이 무효**였다. 둘 다 한 파일만 보면 정상으로 보여 Codex·클로드가 못 잡는 유형이다.
