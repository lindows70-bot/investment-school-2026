...
> **최종 업데이트**: 2026-07-05 — 🚨 글로벌 위기 감지 레이더(투자 리서치) — 시장 버블 4대 지표 실데이터: ①Shiller CAPE(multpl 41.6) ②버핏지표(FRED 시총÷GDP 218%·과거값 실측) ③**선행 PER(FactSet 주간 PDF 실측 20.1·로컬 러너 경유)** ④위험프리미엄(어닝일드−10년물). 종합 Alert 신호등 + 지표별 30년 역사차트 + 반원 게이지 + 쉬운 설명. 하드코딩 거부(제1원칙), FactSet 원본 PDF 직접 파싱(pdf-parse). + 🏛️블랙록 트래커(SEC 13F) + 🚀IPO 하이프 사이클(신규상장주 6단계).
...
- ⏳ **후속(진행 예정)**: sectorConfigs 각 소섹터 **미국3+한국3 종목 보강**(3:3 미달 소섹터 채움, 제미나이 리스트 골격 참고하되 티커 전수 실측 검증=제1원칙). 기존 SectorCanvas 대장주 표에 자동 반영
