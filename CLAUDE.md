...
## 💎 자산군 사이드바 그룹 (2026-07-18) — 전체 자산군 밸런스 진입점

사용자 요청("주요 자산군을 따로 분류해서 관리"). 사이드바 2번째 그룹(📌매일 아래)에 **💎 자산군** 신설 — 📈주식(/research)·📜채권(/bonds)·🏠부동산(/real-estate)·🪙암호화폐(/dashboard?tab=coinlab). 골드 액센트(#d4af37). 기존 여정별 그룹은 그대로(최소 변경·사용자 확정 옵션 A). dashboard 딥링크 ALLOW에 `coinlab` 추가.
- ⚠️ 주식(/research)·부동산(/real-estate)은 각각 🔍종목 확인·🏠부동산 그룹에도 있어 두 곳 동시 하이라이트 — 같은 목적지라 버그 아닌 중복(자산군=밸런스 렌즈·상세 그룹=도구). 밸런스 개요 화면은 사용자 확정으로 미구현(메뉴 분류만).
