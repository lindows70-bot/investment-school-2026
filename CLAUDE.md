...
> **최종 업데이트**: 2026-06-28 — 🏛️ GICS 11 전통 산업 섹터 완성(에너지·소재·산업재·자유소비재·필수소비재·헬스케어·금융·IT·커뮤니케이션·유틸리티·부동산, 테마와 분리된 신규 그룹·US+KR 혼합) + 🌈 비트코인 레인보우 차트(로그 회귀 9밴드) + 📡 자산관리·관심종목 종목별 수급 레이더(차트 하단) + 🧠 AI반도체 섹터에 샌디스크(NAND)·VICR(전력모듈) 추가 + 📈 주가차트 캔들수 통일(1D/1W/1M=60·1Y 월봉 장기, 신규상장주 데이터 한계 정직 표시)
