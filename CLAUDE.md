...
### 데이터·판정
- **`market: 'US'`는 '한국이 아님'** — 유럽·일본·중국 포함. 국기·국적은 **`flagOf()`(`src/lib/marketFlag.ts`) SSOT** 로만 표시한다(origin → 티커 접미사 `.L/.T/.PA/.HK/.SS` → market 순). 직접 `market === 'KR' ? '🇰🇷' : '🇺🇸'` 를 쓰지 마라 — 그 패턴이 17파일 26곳에 퍼져 있었고 두 번 사고가 났다(실적 리포트 상위 50이 전부 일본·승패 해부실 케링 US 오표기).
