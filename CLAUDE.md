...
> **최종 업데이트**: 2026-07-09 — 📋 실전 매매 3종 세트: ①**매매 플랜 카드**(통합추천 전용) — "몇 주를 살까"를 1%리스크룰로 역산(수량=포트×리스크%÷(진입가−ATR손절가)=매매 단위 리스크 패리티), 분할 3단계(신호등 연동)+2R·3R 익절선+갭리스크 경고(명목 15%+). ②**타점 전환 워처** — 매일 08:30 KST 크론이 전 학생 보유 종목 신호등 스캔→전일 대비 🟢돌파/🔴이탈 전환을 대시보드 배너로(본인 보유만 필터). ③막스 시계추에 온도→**권장 현금 밴드**. + 로테이션 시계 전면 리디자인(필 라벨·전역 충돌 해소·축 방향 명확화). 검증: 플랜 12종 수식 소수점 일치·엣지 3건(소수량·눌림가 클램프·갭경고) 수정.
