...
- 🧭 **섹터 페널티(2026-06, 제미나이 피드백)**: 매수 후보 배분이 린치 분류 갭만 보고 섹터 집중은 무시 → 반도체 빼서 또 반도체(삼성·MSFT) 편입해 Tech 71% 잔존. `fillScore`에 `sectorPenalty` 추가(현재 섹터비중 ≥50%→×0.35·≥35%→×0.55·≥20%→×0.8) → 결 다른 섹터(금융·헬스케어) 강제 분산 유도
