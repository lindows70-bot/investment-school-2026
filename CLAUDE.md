...
- **발견성 보완(종목 리서치 종합 매수 판정)**: 통합추천은 green(추세) 위주라 ⬛관망이 잘 안 뜨고, 관망 종목(NVDA 등)은 보유 제외되어 통합추천서 안 보이는 발견성 갭 → `research-verdict`(v3→v4)에 `getEntryTiming` 병렬 fetch 추가, choppy 시 '⬛ 관망(횡보·ADX N)' 주의 근거 노출(보유·추천 무관 아무 종목이나 검색해 확인). green 게이트 동일. 매수 적합+관망 동시일 땐 한줄결론에 'WHAT은 좋음·WHEN은 대기' 명시(모순 오해 차단). 검증: NVDA(yellow·ADX17)=관망 근거 노출·IBB(ADX39)·GS(green)=제외. 참고로 기술차트 판독기는 이미 'ADX 추세 N 박스권' 칩으로 동일 조건 상시 노출 중(아무 종목). ⚠️ **문구 리워딩(화면검증 모순 수정)**: 초판은 '추세 없는 횡보/추세 미형성'이라 써서 **같은 카드의 '🚀 주가 상승추세(50·200 정배열)'·라쉬케 '영선 돌파(추세 확정)'와 정면 모순**(NVDA·오리온). ADX는 추세 *강도*(존재 아님)라 **'⬛ 추세 강도 약함(방향 확신 낮음) — 돌파 신호도 가짜(휩쏘)일 수 있음, 확정 후 진입'**으로 전면 교체 → 상승추세·영선돌파와 상호보완(VPF '가짜 돌파 필터' 개념과 정확히 일치: 돌파 났으나 ADX 약하면 확인 후). 매수카드·TimingBadge·research-verdict(v4→v5)·oneLiner 동시. 검증: NVDA '상승추세(정배열)'+'추세 강도 약함(ADX17)' 일관·모순 문구 잔존 0. 교훈: **ADX<20을 '추세 없음/횡보'로 단정하면 MA 정배열·돌파 신호와 충돌 — '추세 강도 약함'으로 표현해야 다축 트렌드 신호와 공존한다.**
...
- **과유입 오탐 방지 검증**: 파두 → 🌍외국인 매집·INFLOW ✅ / SK하이닉스·삼성전자(외국인 대량 *매도*·개인 매수=🚨개미 독박 구조, majorsNet<0)는 정확히 **중립 유지** — '메이저가 개인 물량 받아냄(유입)'과 '메이저가 팔고 개인이 받음(개미 독박)'을 majorsNet 부호로 정확 분별.
