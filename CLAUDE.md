...
- ⚠️ **발견성 수정(사용자 '화면에서 못 찾겠다')**: 라쉬케를 매매 플랜 카드(**접힌 버튼**) 안에만 넣어 겉면에서 안 보임 → 실사용 안 됨. **TimingBadge(신호등 라인) 겉면에 🎼 칩 노출** — 모든 추천 카드에 상태별: 🎼 첫 눌림목(타점, green)/추세 진행중(green 확립, muted grey — 중복 주장 안 함)/RSI50 돌파·CCI 신호탄·영선 돌파(yellow·red 미확립). 상세 가이드는 매매 플랜 안 유지. 교훈: **기능을 접힌 서브패널에만 넣으면 실사용 0 — 행동 가치 있는 요약은 카드 겉면에.**
