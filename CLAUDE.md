...
- **`/api/tenbagger`** (6h 캐시·maxDuration:60): `?ticker=&market=`(시장 자동추론). **고정 유니버스 하드코딩 없음(제1원칙)** — 기존 엔진 전부 재사용:
  - 🏠 **시총 룸**(작을수록↑, 가중30 — 린치 핵심): stock-info marketCap, **KR은 항상 원화→USD 환산**(Naver marketValueFullRaw는 KRW). <$10B PASS·<$50B PARTIAL·≥$50B FAIL(대형 불가)
  - 📈 고성장(25, earningsGrowth/매출성장 폴백) · 💎 저PEG(20, <0.5 진주) · 🔍 언더커버리지(10, getAnalystSignal reportCount≤3) · 🕵️ 내부자매수(8, getInsiderSignal cluster) · 🛡️ 재무생존력(7, 이자보상배율<1.5=좀비 FAIL)
  - 점수 = 가중 충족도(PASS 1·PARTIAL 0.5·FAIL 0) → 0~100. 🚀**10배거 후보 뱃지**(≥60 & 시총<$30B & 비좀비)
- **`TenbaggerHunter.tsx`**: 종목 입력창 + 7대 체크리스트(✅/⚠️/❌ + 실제값 근거) + 린치 종합 평결 + 빠른검색칩
- ⚠️ **정직 가드레일**: "린치 기준 '충족도'이지 수익 예측 아님 · 텐배거는 희귀·고위험 대부분 실패 · 위성(공격) 자산 소액·분산만 · 교육용". 검증: TEM $9.4B·매출36% / NVDA $5296B 대형불가
...
- **향후(Phase 2 아이디어)**: 코어-위성(Core-Satellite, 기존 asset_role 활용) 리밸런서 통합 — 코어 80%(대형 안정) + 위성 20%(10배거 후보 소액). 학생 피칭→AI검증→유니버스 동적 편입
