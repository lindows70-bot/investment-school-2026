...
> **최종 업데이트**: 2026-07-02 — 🌊 레이 달리오 매크로 사이클 분석 페이지(투자리서치 탭) — ①부채 사이클 6단계 현재위치 FRED 실데이터 자동추정 ②버블 7지표 체크 ③역사 오버레이(현재 vs 2008 vs 2020 실측) ④All Weather(macro-regime 계절 SSOT + Yahoo 20년 백테스트, 2008 방어·2022 실패 정직 노출). 제미나이 설계안을 앱 원칙(Zero-Input·실데이터·SSOT)으로 승격. 빅사이클은 지정학 실시간데이터 부재로 교육 중심.
...
- **`RayDalioAnalysis.tsx`**: INTRO 라이브보드 + 4섹션 + "달리오 정적(All Weather) vs 우리 동적(4계절·퀀트빌더)" 비교 교육. 빅사이클은 지정학 실시간 무료데이터 부재로 **교육 중심**(제국수명 SVG·3대 사이클·8대 국력지표 카드) + 정직 캐비엇.
