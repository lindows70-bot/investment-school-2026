...
> **최종 업데이트**: 2026-07-01 — 🌊 증거 기반 매크로 오버라이드(계절 이론이 실패할 때[섹터성적표 diverge]+빅테크 CapEx 급증[주글라르 surge] 시, 계절 불리 기술주의 적합도를 'Fwd EPS 가속' 증거가 있을 때만 복구 → AI 리밸런싱·통합추천·퀀트빌더가 실제 주도주를 추천에서 밀어내지 않게. 무늬만 AI·역성장은 복구 안 함[반-하이프]. unified-reco SSOT 1곳 수정 → 3개 엔진 자동 상속)
...
- 검증: QQQ 06-22주 실측 O742.02→C706.52가 직전주(O738.10→C740.62)를 완전 감싸 실제 베어리시 인걸핑 확인(사용자 스크린샷 내용과 정확히 일치). 06-29주 반등으로 현재는 해소(none) — 시간 흐름에 따라 신호가 정직하게 사라짐을 확인.
