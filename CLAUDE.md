...
- **발견성 보완(종목 리서치 종합 매수 판정)**: 통합추천은 green(추세) 위주라 ⬛관망이 잘 안 뜨고, 관망 종목(NVDA 등)은 보유 제외되어 통합추천서 안 보이는 발견성 갭 → `research-verdict`(v3→v4)에 `getEntryTiming` 병렬 fetch 추가, choppy 시 '⬛ 관망(횡보·ADX N)' 주의 근거 노출(보유·추천 무관 아무 종목이나 검색해 확인). green 게이트 동일. 매수 적합+관망 동시일 땐 한줄결론에 'WHAT은 좋음·WHEN은 대기' 명시(모순 오해 차단). 검증: NVDA(yellow·ADX17)=관망 근거 노출·IBB(ADX39)·GS(green)=제외. 참고로 기술차트 판독기는 이미 'ADX 추세 N 박스권' 칩으로 동일 조건 상시 노출 중(아무 종목).
