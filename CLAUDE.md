...
> **최종 업데이트**: 2026-06-26 — 🛰️ 양자 테마 인텔리전스(섹터 히트맵·테마지수/MDD·대장주 베타·퓨어플레이·정책바스켓·Pre-IPO proxy·실적 D-day·미니차트, 글로벌 24종 US+KR+JP+EU+CN+CA) + 🌡️ 추세속도(Trend Speed) 맵(MA10 이격도 히트맵, 머니디자인 재현) + 🌐 외국인 종목별 코스피/코스닥 필터 + 🎯 국민연금 매집/축소 종목(DART 5%룰) + 🏛️ 국민연금(연기금) 수급 추적(누적 추세+일별 컬럼) + 🟢 토스 Open API 검증(IP 허용목록 제약 → 고정 IP 실거래 러너 토대 보존)
