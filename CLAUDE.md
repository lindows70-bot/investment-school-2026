...
- **🗺️ 서울 자치구 평당가 히트맵**(`/api/re-map`·`SeoulAptMap.tsx`·`public/geo/seoul-gu.json` 25개구 경계): 구별 최근 3개월 매매 ㎡당 중위가×3.3=평당가 색칠(react-simple-maps 재사용) + 호버 평당가 + 클릭=단지 리서치 로드 + 순위. **캐시 전용 설계**(외부 콜 0 — data.go.kr 한도 보호): 이미 수집된 구만 색칠, 미수집=회색·클릭 시 수집 후 자동 색칠(수집될수록 지도가 채워지는 구조). 단지별 핀은 좌표 지오코딩 키 필요라 정직 보류. 검증: 강남 1.2억>송파 0.8>노원 0.35(현실 서열 정합·839~1542건 표본). ⚠️ **병렬 getCache 서버리스 silent 실패(중요 함정)**: 25개구×3개월=75회 `getCache`를 Promise.all로 돌리니 **Vercel에서 첫 구(강남)만 성공하고 나머지 전부 조용히 null**(getCache가 에러를 삼킴 — 로컬은 9/9 정상, prod REST 직접 조회도 200 정상이라 데이터 문제 아님). 원인은 요청당 createClient 75개 동시 생성의 fetch 실패로 추정 → **단일 `.in('key',keys)` 일괄 쿼리로 재설계**해 해소. 교훈: **서버리스에서 키별 getCache 대량 병렬 금지 — 여러 키는 in() 한 방이 정답**(getCache의 graceful 삼킴이 이런 실패를 침묵시키므로 '일부만 되는' 증상이면 이 패턴 의심).
