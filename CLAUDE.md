...
- ⚠️ **아파트별 현재 단계(2차 미완)**: OA-22856 xlsx에 추진단계 있으나 **다운로드가 브라우저 JS 전용**(nio_download.do가 세션+seq 서버검증 → curl '잘못된 접근' 에러, data.go.kr도 세션 필요 빈응답 — 자동화 불가 실측). python openpyxl 파싱은 가능하나 파일 확보가 관건 → **사용자 수동 다운로드 후 공유 → 정적 JSON 파싱 join** 또는 로컬 러너(KRX/토스 패턴) 필요. 교훈: **서울 열린데이터광장 '파일' 데이터셋은 nio_download.do 폼이 세션+seq 검증이라 curl 자동화 불가 — OpenAPI(json)만 자동화 가능**.
