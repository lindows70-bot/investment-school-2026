...
> **최종 업데이트**: 2026-06-10 — 🎛️ AI 포트폴리오 운용 본부(통합 3축 추천+본부장 브리핑) 완성
...
- ⭐ **쌍둥이 지수 차용(2026-06-10)**: 해외형 KR ETF 종목비중 해결 — 사용자 제보(운용사 사이트) 라이브 검증 결과 TIGER/KODEX SPA(숨은 API 미발견)·KRX(LOGOUT) 전부 실패 → `etfBaseIndex`(추종지수)가 표준지수(S&P500→SPY·NASDAQ100→QQQ·필반→SOXX·다우→DIA·러셀2000→IWM, 정확 매칭만)면 **검증된 US 쌍둥이 ETF 구성비중 차용**(동일 지수 실측값, 새 크롤러 0). 비표준지수(테크TOP10 INDXX 등)는 정직하게 섹터만 유지. weightSource(native/twin)·twinTicker. etf-comp v4·xray v3. 검증 5/5(TIGER S&P500→SPY NVDA 7.9%)
