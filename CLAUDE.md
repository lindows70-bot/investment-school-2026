...
- **LLM 요약의 금액은 '검사하는 필드'만 안전하다** — 매출·영업이익만 검산하던 시절 가이던스의 "$13 billion → 13억"(×1/10)이 화면까지 나갔다. 검산기를 넓히자 숨은 자릿수 오류 4종이 더 나왔다. 자유 서식 필드도 원문 '$X billion/million/trillion' 명시값(정수·범위 포함)과 대조하라.
