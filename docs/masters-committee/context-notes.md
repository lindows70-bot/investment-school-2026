...
## 검증에서 발견한 것 (2026-08-06)
- 독립 검산 3종목(NVDA·005930 dev, CVX prod): 판정 산식(fail>=2 불통과 / fail=1 or
  warn>=2 회색) 손검산 12/12 일치, 매수 밴드 산식(x0.70/x0.85) 일치.
- NVDA = 회색 — 버핏 불통과(FCF수익률 0.9%·안전마진 -87%)와 멍거·단요핑 통과가
  갈렸고, 교차 반박이 "PEG만 보고 현금 창출력을 못 봤다"처럼 판정표 근거로만 생성됨.
  양비론 파괴 구조가 의도대로 작동.
- 005930 = 4/4 통과 + 매수구간 37.4만~45.4만(현재가 24.6만 — 구간 아래).
- CVX = 회색 — 멍거 인버전④(역-DCF demanding)를 fail 로 정확히 잡되 implied 11%<20
  이라 레드라인은 미발동(경기순환주 과잉 처벌 방지 설계 확인). 단, DCF 매수구간
  60~73 vs 현재가 187 — 경기순환주는 사이클 고점 이익 기준 DCF 라 구간이 극단적으로
  보수적일 수 있다. 캐비엇으로 커버하나 v2 에서 순환주 정규화 검토 여지.
- 캐시 24h(계획의 7d 에서 변경) — 안전마진·어닝일드·52주 위치가 가격 의존이라
  일 단위가 정직하다.
