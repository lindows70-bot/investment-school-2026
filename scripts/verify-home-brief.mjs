...
check('움직임: failed>0 → 구분자 뒤 따로 "일부 종목은 확인 못 함"(warn)', part(bFail.mine, '일부 종목은 확인 못 함')?.tone === 'warn' && T(bFail.mine).endsWith('5% 넘게 움직인 종목 없음 · 일부 종목은 확인 못 함'))
const bFailMv = run({ movers: { held: held3, failed: 1 } })
check('움직임: failed>0 + 종목 있음 → 마지막 종목 뒤에 구분자가 먼저 온다(종목에 붙지 않음)', T(bFailMv.mine).endsWith('TIGER 코리아원자력 −5.8% 외 1 · 일부 종목은 확인 못 함'))
...
check('일정: 실적 가장 가까운 1건(중복·지난 것 무시)', T(run({ fomcDates: ['2026-10-28'], events: ev }).upcoming) === '다가오는 일정: 9/29 마이크론 실적 · 10/28 FOMC 금리 결정')
check('일정: 날짜순 — FOMC 가 먼저면 FOMC 먼저', T(run({ fomcDates: ['2026-09-28'], events: ev }).upcoming) === '다가오는 일정: 9/28 FOMC 금리 결정 · 9/29 마이크론 실적')
